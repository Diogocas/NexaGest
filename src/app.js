// NexaGest 9.1.1 - Limpeza da Base e Atualização Automática Ajustada
...
function commercialUpdateStatusView(){
  ...
  if(!info || !Object.keys(info).length)return '<div class="update-status-box neutral"><b>Nenhuma verificação realizada</b><span>Informe uma URL de manifesto ou use o manifesto local em GitHub Releases/latest.yml.</span></div>';
...
function setupUpdaterEvents(){
  if(updaterEventsBound||!window.nexagest?.onUpdaterEvent)return;
  updaterEventsBound=true;
  window.nexagest.onUpdaterEvent((event)=>{
    db.settings.lastUpdateInfo={...(db.settings.lastUpdateInfo||{}),...(event||{})};
    db.settings.lastUpdateCheckAt=event?.checkedAt||new Date().toISOString();
    save();
    if(event?.channel==='available'||event?.updateAvailable){
      if(startupUpdateCheckDone && event._prompted)return;
      const latest=event.latestVersion||event.version||'nova versão';
      setTimeout(()=>{
        if(confirm('Nova versão do NexaGest disponível: '+latest+'\n\nDeseja baixar e instalar agora?')){
          downloadAvailableUpdate();
        }
      },250);
    }
    if(event?.channel==='error'&&String(event.error||'').toLowerCase().includes('github')){
      console.warn('Atualização via GitHub falhou. Se o repositório for privado, use um repositório público separado apenas para Releases.', event.error);
    }
  });
}
async function autoCheckUpdatesOnStartup(){
  ...
  if(db.settings.autoUpdate===false)return;
  try{
    const r=await window.nexagest?.commercialCheckUpdate?.({
      manifestUrl:db.settings.updateManifestUrl||'',
      githubOwner:db.settings.githubOwner||'Diogocas',
      githubRepo:db.settings.githubRepo||'NexaGest'
    });
    db.settings.lastUpdateInfo=r||{};
    db.settings.lastUpdateCheckAt=new Date().toISOString();
    save();
    if(r?.updateAvailable){
      setTimeout(()=>{
        if(confirm('Nova versão do NexaGest disponível: '+(r.latestVersion||r.version||'')+'\n\n'+(r.notes||'')+'\n\nDeseja baixar e instalar agora?')){
          downloadAvailableUpdate();
        }
      },500);
    }else if(r?.ok===false){
      console.warn('Verificação automática de atualização falhou:', r.error||r.status||r);
    }
  }catch(e){
    console.warn('Verificação automática de atualização falhou:', e);
  }
}

async function boot(){db=await loadSqliteDatabase();await refreshCompanyInfo();runAutoBackup();setupUpdaterEvents();app();setTimeout(autoCheckUpdatesOnStartup,1600)}
boot();
