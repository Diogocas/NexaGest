// NexaGest 1.0.0 - Primeira versão comercial
...
function updateLiveClock(){
  const current=formatTopbarDateTime();
  document.querySelectorAll('[data-live-date]').forEach(el=>{el.textContent=current.date});
  document.querySelectorAll('[data-live-time]').forEach(el=>{el.textContent=current.time});
  document.querySelectorAll('[data-license-global-warning]').forEach(el=>{
    const data=licenseGlobalWarningData();
    if(!data){ el.className='license-global-warning hidden'; el.textContent=''; el.title=''; return; }
    el.className='license-global-warning '+data.level;
    el.textContent=data.text;
    el.title=data.title||data.text;
  });
  refreshLicenseCountdownLabels();
}
...
function liveDateTimeHtml(extraClass=''){
  const current=formatTopbarDateTime();
  const warning=licenseGlobalWarningData();
  const warningHtml=warning?`<span class="license-global-warning ${warning.level}" data-license-global-warning title="${esc(warning.title||warning.text)}">${esc(warning.text)}</span>`:`<span class="license-global-warning hidden" data-license-global-warning></span>`;
  return `<span class="live-clock premium-clock ${extraClass}" aria-label="Data, hora atual e status de licença"><span class="clock-chip date-chip"><span class="clock-icon">📅</span><span data-live-date>${current.date}</span></span><span class="clock-divider"></span><span class="clock-chip time-chip"><span class="clock-pulse"></span><span class="clock-icon">🕒</span><strong data-live-time>${current.time}</strong></span>${warningHtml}</span>`;
}
...
const ROLE_PERMISSIONS={
  Administrador:['dashboard','pdv','products','stock','clients','finance','purchases','nfe','suppliers','quotes','deliveries','labels','reports','premium','cloud','ai','users','settings'],
  Caixa:['pdv','clients','quotes','deliveries'],
  Estoque:['products','stock','purchases','nfe','suppliers','labels','cloud','ai']
};
...
function layout(){if(!can(page))page=firstAllowedPage();if(cashierMode&&page==='pdv')return cashierLayout();let allowed=nav.filter(n=>can(n[0]));return `<div class="app"><aside class="sidebar"><div class="brand">${logo()}<div><b>NexaGest</b><span>${esc(db.settings.company)}</span></div></div><div class="nav">${allowed.map(n=>`<button data-page="${n[0]}" class="${page===n[0]?'active':''}"><span>${n[1]}</span>${n[2]}</button>`).join('')}</div><div class="sidebar-foot"><div class="small">Logado como <b>${esc(db.session.name)}</b><br><span>${esc(db.session.role)}</span></div><button class="ghost full" id="lockSession">🔒 Bloquear</button><button class="ghost full" id="switchUser">🔄 Trocar usuário</button><button class="ghost full" id="logout">🚪 Sair</button></div></aside><main class="main"><div class="topbar"><div><h1>${nav.find(n=>n[0]===page)?.[2]}</h1><p class="muted topbar-clock">${liveDateTimeHtml()}</p></div><div class="row no-print">${page==='pdv'?'<button class="ok" id="enterCashierMode">🧾 Modo Operador</button>':''}${canSwitchCompany()?`<button class="ghost company-switch-top" id="openCompanySwitcher">🏢 ${esc(currentCompanyName())}</button>`:''}<button class="ghost" id="openCommandPalette">⌘ Ctrl+K</button><button class="ghost" id="lockSessionTop">🔒 Bloquear</button><button class="ghost" id="switchUserTop">🔄 Trocar usuário</button><button class="ghost" id="themeToggle">${db.settings.theme==='dark'?'☀️ Claro':'🌙 Escuro'}</button></div></div>${views[page]()}</main></div>`}
function cashierLayout(){return `<div class="cashier-mode app"><main class="cashier-main"><div class="cashier-topbar no-print"><div class="cashier-brand">${logo()}<div><b>Modo Operador de Caixa</b><span>${esc(db.settings.company)}</span></div></div><div class="cashier-status"><span>Operador: <b>${esc(db.session?.name||'')}</b></span><span>Perfil: <b>${esc(db.session?.role||'')}</b></span><span>${liveDateTimeHtml('cashier-clock')}</span></div><div class="cashier-actions">${canSwitchCompany()?`<button class="ghost company-switch-top" id="openCompanySwitcher">🏢 ${esc(currentCompanyName())}</button>`:''}<button class="ghost" id="openCommandPalette">⌘ Ctrl+K</button><button class="ghost" id="focusPdvSearch">Focar busca</button><button class="ghost" id="lockSessionTop">🔒 Bloquear</button><button class="ghost" id="switchUserTop">🔄 Trocar usuário</button><button class="danger" id="exitCashierMode">Sair do modo caixa</button></div></div>${views.pdv()}</main></div>`}
...
function dashboardQuickActions(){
  ...
  return `<div class="panel dash-quick"><div class="between"><h3>Ações rápidas</h3><span class="pill">Dashboard</span></div><div class="dash-action-grid">${actions.map(a=>`<button class="dash-action" data-dash-go="${a[0]}"><b>${a[1]}</b><span>${a[2]}</span><small>${a[3]}</small></button>`).join('')}</div></div>`
}
...
function dashboardCommercialIntelligence(d){
  const now=new Date();
  const soldAll=new Map();
  salesValid().forEach(s=>(s.items||[]).forEach(i=>{
    const p=product(i.id);
    const qty=Number(i.qty||0);
    const price=Number(i.price||p.sale||0);
    const cost=Number(i.cost||p.cost||0);
    const cur=soldAll.get(i.id)||{p,qty:0,revenue:0,profit:0,last:''};
    cur.qty+=qty;
    cur.revenue+=price*qty;
    cur.profit+=(price-cost)*qty;
    const day=(s.date||'').slice(0,10);
    if(day>cur.last)cur.last=day;
    soldAll.set(i.id,cur);
  }));
  const active=(db.products||[]).filter(p=>p.active!==false);
  const stopped=active.map(p=>{
    const info=soldAll.get(p.id);
    const last=info?.last||'';
    const days=last?Math.floor((now-new Date(last+'T00:00:00'))/86400000):9999;
    return {p,last,days,stock:Number(p.stock||0)};
  }).filter(x=>x.stock>0&&x.days>=30).sort((a,b)=>b.days-a.days).slice(0,5);
  const profitable=[...soldAll.values()].filter(x=>x.profit>0).sort((a,b)=>b.profit-a.profit).slice(0,5);
  const vipMap=new Map();
  salesValid().forEach(s=>{let id=s.clientId||'balcao',c=client(id),cur=vipMap.get(id)||{name:c.name||'Balcão',total:0,count:0,last:''};cur.total+=Number(s.total||0);cur.count++;let day=(s.date||'').slice(0,10);if(day>cur.last)cur.last=day;vipMap.set(id,cur)});
  const vip=[...vipMap.values()].filter(x=>x.name!=='Balcão').sort((a,b)=>b.total-a.total).slice(0,5);
  let suggestions=[];
  if(d.low.length)suggestions.push(['🔴','Comprar estoque crítico',`${d.low.length} produto(s) abaixo ou no mínimo definido.`,'stock']);
  if(stopped.length)suggestions.push(['📦','Revisar produtos parados',`${stopped.length} item(ns) com estoque e sem venda há mais de 30 dias.`,'products']);
  if(profitable[0])suggestions.push(['💎','Destacar produto lucrativo',`${profitable[0].p.name} gerou lucro estimado de ${money(profitable[0].profit)}.`,'products']);
  if(vip[0])suggestions.push(['👑','Atender cliente VIP',`${vip[0].name} já comprou ${money(vip[0].total)} no histórico.`,'clients']);
  if(d.goal&&d.goalPct<70)suggestions.push(['🎯','Acelerar meta mensal',`Falta ${money(Math.max(0,d.goal-d.monthRev))}; necessário por dia: ${money(d.needPerDay)}.`,'reports']);
  if(!suggestions.length)suggestions.push(['✅','Sem ação urgente','Os principais indicadores estão sob controle agora.','dashboard']);
  return `<div class="panel dashboard-ai-panel"><div class="between"><div><h3>🧠 Inteligência Comercial</h3><p class="muted small">Leitura automática de produtos parados, margem, estoque, clientes VIP e ações sugeridas.</p></div></div><div class="grid three smart-insights"><div><h4>Produtos parados</h4>${stopped.length?table(['Produto','Estoque','Última venda'],stopped.map(x=>[esc(x.p.name),x.stock,x.last?`${x.days} dias`:'Nunca vendeu'])):'<p class="muted">Nenhum produto parado com estoque.</p>'}</div><div><h4>Mais lucrativos</h4>${profitable.length?table(['Produto','Qtd.','Lucro'],profitable.map(x=>[esc(x.p.name),Number(x.qty||0),`<b class="good">${money(x.profit)}</b>`])):'<p class="muted">Sem lucro calculado ainda.</p>'}</div><div><h4>Clientes VIP</h4>${vip.length?table(['Cliente','Compras','Total'],vip.map(x=>[esc(x.name),x.count,`<b>${money(x.total)}</b>`])):'<p class="muted">Sem clientes VIP calculados ainda.</p>'}</div></div><div class="dashboard-suggestions"><h4>Sugestões automáticas</h4>${suggestions.map(x=>`<button data-dash-go="${x[3]}"><b>${x[0]}</b><span>${esc(x[1])}</span><small>${esc(x[2])}</small></button>`).join('')}</div></div>`
}


function dashboardSmartScore(d){
  const margin=d.monthRev?Math.round((d.net/d.monthRev)*100):0;
  const stockPenalty=Math.min(25,(d.low||[]).length*4);
  const cashPenalty=(d.receivables||[]).length?Math.min(15,(d.receivables||[]).length*2):0;
  const salesScore=Math.min(35,d.goal?Math.round(d.goalPct*.35):((d.transactions||0)?28:8));
  const profitScore=Math.max(0,Math.min(30,margin+18));
  const score=Math.max(0,Math.min(100,salesScore+profitScore+35-stockPenalty-cashPenalty));
  const cls=score>=75?'good':score>=50?'warn':'bad';
  const label=score>=75?'Negócio saudável':score>=50?'Precisa de atenção':'Ação urgente';
  const checks=[
    ['Vendas',d.transactions?`${d.transactions} venda(s) no período`:'Sem vendas no período',d.transactions?'good':'warn'],
    ['Margem',`${margin}% de margem líquida estimada`,margin>=15?'good':margin>=0?'warn':'bad'],
    ['Estoque',d.low.length?`${d.low.length} produto(s) críticos`:'Estoque sem alerta crítico',d.low.length?'bad':'good'],
    ['Financeiro',d.receivables.length?`${d.receivables.length} conta(s) em aberto`:'Sem recebíveis pendentes',d.receivables.length?'warn':'good']
  ];
  return `<div class="panel smart-score-panel"><div class="smart-score-main"><div><span class="pill ${cls}">${label}</span><h3>Score comercial</h3><p class="muted small">Nota automática baseada em vendas, margem, estoque e financeiro.</p></div><strong class="${cls}">${score}</strong></div><div class="smart-score-track"><i style="width:${score}%"></i></div><div class="smart-score-checks">${checks.map(c=>`<div class="${c[2]}"><span>${esc(c[0])}</span><b>${esc(c[1])}</b></div>`).join('')}</div></div>`;
}
function dashboardReorderPlan(d){
  const last30=salesBetween(dateKeyOffset(-29),today());
  const qtyMap=new Map();
  last30.forEach(s=>(s.items||[]).forEach(i=>qtyMap.set(i.id,(qtyMap.get(i.id)||0)+Number(i.qty||0))));
  const rows=(db.products||[]).filter(p=>p.active!==false).map(p=>{
    const sold=Number(qtyMap.get(p.id)||0), daily=sold/30, stock=Number(p.stock||0), min=Number(p.min||0);
    const daysLeft=daily?Math.floor(stock/daily):999;
    const suggested=Math.max(0,Math.ceil((daily*15+min)-stock));
    let priority=stock<=min?'Alta':daysLeft<=7?'Média':suggested>0?'Baixa':'OK';
    return {p,sold,daily,stock,min,daysLeft,suggested,priority};
  }).filter(x=>x.priority!=='OK' || x.suggested>0 || x.stock<=x.min).sort((a,b)=>({Alta:0,Média:1,Baixa:2,OK:3}[a.priority]-{Alta:0,Média:1,Baixa:2,OK:3}[b.priority]) || b.suggested-a.suggested).slice(0,8);
  return `<div class="panel reorder-panel"><div class="between"><div><h3>Plano automático de compras</h3><p class="muted small">Sugestão baseada nas vendas dos últimos 30 dias, estoque atual e estoque mínimo.</p></div><button class="ghost" data-dash-go="purchases">Ir para compras</button></div>${rows.length?table(['Prioridade','Produto','Estoque','Vendido 30d','Cobertura','Comprar'],rows.map(x=>[`<span class="pill ${x.priority==='Alta'?'bad':x.priority==='Média'?'warn':'good'}">${x.priority}</span>`,esc(x.p.name),`${x.stock} / mín. ${x.min}`,x.sold,x.daysLeft===999?'Sem giro':`${x.daysLeft} dia(s)`,x.suggested?`<b>${x.suggested} un</b>`:'-'])):'<p class="muted">Nenhuma sugestão de compra no momento.</p>'}</div>`;
}
function dashboardOpportunityPanel(d){
  const sales=salesValid();
  const byClient=new Map();
  sales.forEach(s=>{const id=s.clientId||''; if(!id)return; const c=client(id); const cur=byClient.get(id)||{c,total:0,count:0,last:''}; cur.total+=Number(s.total||0); cur.count++; const day=(s.date||'').slice(0,10); if(day>cur.last)cur.last=day; byClient.set(id,cur)});
  const dormant=[...byClient.values()].filter(x=>x.last&&Math.floor((new Date()-new Date(x.last+'T00:00:00'))/86400000)>=45).sort((a,b)=>b.total-a.total).slice(0,5);
  const basket=d.transactions?d.monthRev/d.transactions:0;
  const highTicket=[...byClient.values()].filter(x=>x.count&&x.total/x.count>basket&&x.total>0).sort((a,b)=>(b.total/b.count)-(a.total/a.count)).slice(0,5);
  return `<div class="grid two"><div class="panel opportunity-panel"><h3>Clientes para reativar</h3>${dormant.length?table(['Cliente','Última compra','Total'],dormant.map(x=>[esc(x.c.name),br(x.last),money(x.total)])):'<p class="muted">Nenhum cliente importante parado há mais de 45 dias.</p>'}</div><div class="panel opportunity-panel"><h3>Oportunidades de venda</h3>${highTicket.length?table(['Cliente','Ticket médio','Compras'],highTicket.map(x=>[esc(x.c.name),money(x.total/x.count),x.count])):'<p class="muted">Ainda sem dados suficientes para oportunidades.</p>'}</div></div>`;
}
function dashboardActionCenter(d){
  const actions=[];
  if(d.low.length)actions.push(['Comprar estoque crítico',`${d.low.length} produto(s) no mínimo ou abaixo do mínimo.`, 'Alta','stock']);
  if(d.goal&&d.goalPct<80)actions.push(['Impulsionar vendas',`Meta em ${d.goalPct}%. Falta ${money(Math.max(0,d.goal-d.monthRev))}.`, 'Média','pdv']);
  if(d.receivables.length)actions.push(['Cobrar pendências',`${d.receivables.length} conta(s) a receber em aberto.`, 'Média','finance']);
  if(!d.openCash.length)actions.push(['Abrir caixa', 'Nenhum caixa aberto agora.', 'Baixa','pdv']);
  if(!actions.length)actions.push(['Manter acompanhamento','Indicadores principais dentro do esperado.', 'OK','reports']);
  return `<div class="panel action-center-panel"><div class="between"><div><h3>Central de ações recomendadas</h3><p class="muted small">Lista objetiva do que o gestor deve olhar primeiro.</p></div></div><div class="action-center-list">${actions.map(a=>`<button data-dash-go="${a[3]}"><b>${esc(a[0])}</b><span>${esc(a[1])}</span><em class="${a[2]==='Alta'?'bad':a[2]==='Média'?'warn':'good'}">${esc(a[2])}</em></button>`).join('')}</div></div>`;
}




function biPct(v,total){return total?Math.round((Number(v||0)/total)*1000)/10:0}
function biProgress(value,total,cls='good'){let pct=Math.max(0,Math.min(100,biPct(value,total)));return `<div class="bi-progress"><i class="${cls}" style="width:${pct}%"></i><span>${pct.toFixed(1).replace('.',',')}%</span></div>`}
function biProData(d){
  const sales=d.monthSales||[], prev=d.prevMonthSales||[], products=db.products||[];
  const revenue=d.monthRev||0, cost=d.cost||0, expenses=d.exp||0, profit=d.net||0;
  const margin=revenue?((profit/revenue)*100):0, gross=revenue-cost, grossMargin=revenue?((gross/revenue)*100):0;
  const prevRevenue=d.prevRev||0, prevProfit=d.prevNet||0;
  const productMap=new Map();
  sales.forEach(s=>(s.items||[]).forEach(i=>{const p=product(i.id),qty=Number(i.qty||0),price=Number(i.price||p.sale||0),unitCost=Number(i.cost??p.cost??0);const cur=productMap.get(i.id)||{p,qty:0,revenue:0,cost:0,profit:0};cur.qty+=qty;cur.revenue+=price*qty;cur.cost+=unitCost*qty;cur.profit+=(price-unitCost)*qty;productMap.set(i.id,cur)}));
  const productRows=[...productMap.values()].sort((a,b)=>b.revenue-a.revenue);
  let acc=0; const abc=productRows.map((x,i)=>{acc+=x.revenue;let pct=biPct(acc,revenue);let curve=pct<=80?'A':pct<=95?'B':'C';return{...x,rank:i+1,accPct:pct,curve}});
  const abcSummary={A:abc.filter(x=>x.curve==='A'),B:abc.filter(x=>x.curve==='B'),C:abc.filter(x=>x.curve==='C')};
  const clientMap=new Map();
  sales.forEach(s=>{let id=s.clientId||'balcao',c=client(id),cur=clientMap.get(id)||{c,name:c.name||'Balcão',count:0,revenue:0,profit:0};cur.count++;cur.revenue+=Number(s.total||0);cur.profit+=Number(s.profit??(Number(s.total||0)-Number(s.cost||0)));clientMap.set(id,cur)});
  const clientRows=[...clientMap.values()].sort((a,b)=>b.revenue-a.revenue);
  let clientAcc=0; const clientAbc=clientRows.map((x,i)=>{clientAcc+=x.revenue;let pct=biPct(clientAcc,revenue);let curve=pct<=80?'A':pct<=95?'B':'C';return{...x,rank:i+1,accPct:pct,curve}});
  const week=['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'];
  const heat=Array.from({length:7},(_,day)=>Array.from({length:15},(_,h)=>({day,hour:h+7,value:0,count:0})));
  sales.forEach(s=>{let dt=new Date(s.date||Date.now()),day=dt.getDay(),h=dt.getHours();if(h>=7&&h<=21){let cell=heat[day][h-7];cell.value+=Number(s.total||0);cell.count++}});
  const maxHeat=Math.max(1,...heat.flat().map(x=>x.value));
  const bestHeat=heat.flat().sort((a,b)=>b.value-a.value)[0]||{day:0,hour:0,value:0,count:0};
  const monthRows=[];for(let i=5;i>=0;i--){let k=isoMonthKey(-i);let cur=salesValid().filter(s=>(s.date||'').slice(0,7)===k);let exp=(db.expenses||[]).filter(e=>(e.date||'').slice(0,7)===k);let rev=sum(cur,'total'),cst=sum(cur,'cost'),ex=sum(exp,'value'),net=rev-cst-ex;monthRows.push({label:monthLabel(k),revenue:rev,profit:net,expenses:ex})}
  const stockValue=products.reduce((a,p)=>a+Number(p.stock||0)*Number(p.cost||0),0);
  const saleStockValue=products.reduce((a,p)=>a+Number(p.stock||0)*Number(p.sale||0),0);
  const avgTicket=sales.length?revenue/sales.length:0;
  const turnover=stockValue?cost/stockValue:0;
  return{revenue,cost,expenses,profit,margin,gross,grossMargin,prevRevenue,prevProfit,abc,abcSummary,clientAbc,heat,maxHeat,bestHeat,monthRows,stockValue,saleStockValue,avgTicket,turnover};
}
function biMiniBars(rows,key,labelKey='label'){let max=Math.max(1,...rows.map(r=>Number(r[key]||0)));return `<div class="bi-mini-bars">${rows.map(r=>`<div><span>${esc(r[labelKey])}</span><i style="width:${Math.max(4,Number(r[key]||0)/max*100)}%"></i><b>${money(r[key])}</b></div>`).join('')}</div>`}
function dashboardBiProPanel(d){
  const bi=biProData(d);
  const heatRows=bi.heat.map((row,day)=>`<div class="bi-heat-row"><b>${['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][day]}</b>${row.map(c=>`<span class="bi-heat-cell" style="opacity:${.22+(c.value/bi.maxHeat)*.78}" title="${['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][day]} ${String(c.hour).padStart(2,'0')}h • ${money(c.value)} • ${c.count} venda(s)"></span>`).join('')}</div>`).join('');
  const abcRows=bi.abc.slice(0,10).map(x=>[`<span class="pill ${x.curve==='A'?'good':x.curve==='B'?'warn':''}">${x.curve}</span>`,esc(x.p.name),money(x.revenue),money(x.profit),biProgress(x.revenue,bi.revenue,x.curve==='A'?'good':x.curve==='B'?'warn':'')]);
  const clientRows=bi.clientAbc.slice(0,8).map(x=>[`<span class="pill ${x.curve==='A'?'good':x.curve==='B'?'warn':''}">${x.curve}</span>`,esc(x.name),x.count,money(x.revenue),money(x.profit)]);
  const best=`${['Domingo','Segunda','Terça','Quarta','Quinta','Sexta','Sábado'][bi.bestHeat.day]||'-'} ${String(bi.bestHeat.hour||0).padStart(2,'0')}h`;
  return `<div class="panel bi-pro-panel"><div class="between"><div><h3>📊 BI</h3><p class="muted small">Painel executivo com Curva ABC, heatmap de vendas, comparativos e indicadores gerenciais.</p></div></div><div class="grid cards bi-executive-cards"><div class="card"><span>Receita</span><b>${money(bi.revenue)}</b><small>${trendBadge(bi.revenue,bi.prevRevenue)}</small></div><div class="card"><span>Lucro líquido</span><b class="${bi.profit>=0?'good':'bad'}">${money(bi.profit)}</b><small>Margem ${bi.margin.toFixed(1).replace('.',',')}%</small></div><div class="card"><span>Margem bruta</span><b>${bi.grossMargin.toFixed(1).replace('.',',')}%</b><small>${money(bi.gross)} antes das despesas</small></div><div class="card"><span>Valor em estoque</span><b>${money(bi.stockValue)}</b><small>Venda potencial ${money(bi.saleStockValue)}</small></div><div class="card"><span>Giro estimado</span><b>${bi.turnover.toFixed(2).replace('.',',')}x</b><small>Custo vendido / estoque</small></div></div><div class="grid two"><div class="panel nested bi-exec-chart"><div class="between"><h4>Comparativo executivo mensal</h4><span class="pill">6 meses</span></div>${biMiniBars(bi.monthRows,'revenue')}</div><div class="panel nested"><div class="between"><h4>Indicadores</h4><span class="pill ${bi.margin>=15?'good':bi.margin>=0?'warn':'bad'}">${bi.margin>=15?'Bom':bi.margin>=0?'Atenção':'Crítico'}</span></div><div class="bi-indicators"><div><span>Ticket médio</span><b>${money(bi.avgTicket)}</b></div><div><span>Despesas / Receita</span><b>${bi.revenue?(bi.expenses/bi.revenue*100).toFixed(1).replace('.',','):0}%</b></div><div><span>Produtos classe A</span><b>${bi.abcSummary.A.length}</b></div><div><span>Melhor horário</span><b>${esc(best)}</b></div></div></div></div><div class="grid two"><div class="panel nested"><div class="between"><h4>Curva ABC de produtos</h4><span class="pill">Top 10</span></div>${abcRows.length?table(['ABC','Produto','Receita','Lucro','Part.'],abcRows):'<p class="muted">Sem dados para Curva ABC no período.</p>'}</div><div class="panel nested"><div class="between"><h4>Curva ABC de clientes</h4><span class="pill">Top clientes</span></div>${clientRows.length?table(['ABC','Cliente','Compras','Receita','Lucro'],clientRows):'<p class="muted">Sem clientes para classificar no período.</p>'}</div></div><div class="panel nested bi-heatmap"><div class="between"><h4>Heatmap de vendas por dia e horário</h4><span class="pill">pico: ${esc(best)}</span></div><div class="bi-heat-hours">${Array.from({length:15},(_,i)=>`<span>${String(i+7).padStart(2,'0')}h</span>`).join('')}</div>${heatRows}<p class="muted small">Quanto mais forte a célula, maior o faturamento naquele horário.</p></div></div>`;
}

function dashboardPredictiveData(d){
  const nowKey=today();
  const sales30=salesBetween(dateKeyOffset(-29),nowKey);
  const salesPrev30=salesBetween(dateKeyOffset(-59),dateKeyOffset(-30));
  const qty30=new Map(), qtyPrev=new Map(), rev30=new Map(), revPrev=new Map();
  sales30.forEach(s=>(s.items||[]).forEach(i=>{let p=product(i.id),q=Number(i.qty||0),v=Number(i.price||p.sale||0)*q;qty30.set(i.id,(qty30.get(i.id)||0)+q);rev30.set(i.id,(rev30.get(i.id)||0)+v)}));
  salesPrev30.forEach(s=>(s.items||[]).forEach(i=>{let p=product(i.id),q=Number(i.qty||0),v=Number(i.price||p.sale||0)*q;qtyPrev.set(i.id,(qtyPrev.get(i.id)||0)+q);revPrev.set(i.id,(revPrev.get(i.id)||0)+v)}));
  const stockForecast=(db.products||[]).filter(p=>p.active!==false).map(p=>{let sold=Number(qty30.get(p.id)||0),daily=sold/30,stock=Number(p.stock||0),min=Number(p.min||0),daysLeft=daily?Math.floor(stock/daily):999,suggested=Math.max(0,Math.ceil((daily*20+min)-stock));let status=stock<=min?'Crítico':daysLeft<=7?'Acaba em breve':daysLeft<=15?'Atenção':'OK';return{p,sold,daily,stock,min,daysLeft,suggested,status}}).filter(x=>x.status!=='OK'||x.suggested>0).sort((a,b)=>(a.daysLeft-b.daysLeft)||b.suggested-a.suggested).slice(0,6);
  const rising=[], falling=[];
  (db.products||[]).filter(p=>p.active!==false).forEach(p=>{let cur=Number(qty30.get(p.id)||0),prev=Number(qtyPrev.get(p.id)||0);if(cur===0&&prev===0)return;let pct=prev?((cur-prev)/prev)*100:(cur>0?100:0);let row={p,cur,prev,pct,revenue:Number(rev30.get(p.id)||0)};if(pct>=25&&cur>0)rising.push(row);if(pct<=-25&&prev>0)falling.push(row)});
  rising.sort((a,b)=>b.pct-a.pct);falling.sort((a,b)=>a.pct-b.pct);
  const byClient=new Map();
  salesValid().forEach(s=>{if(!s.clientId)return;let c=client(s.clientId),cur=byClient.get(s.clientId)||{c,total:0,count:0,last:'',avgDays:0,dates:[]};cur.total+=Number(s.total||0);cur.count++;let day=(s.date||'').slice(0,10);if(day){cur.dates.push(day);if(day>cur.last)cur.last=day}byClient.set(s.clientId,cur)});
  const atRisk=[...byClient.values()].map(x=>{let sorted=[...new Set(x.dates)].sort();let gaps=[];for(let i=1;i<sorted.length;i++)gaps.push(Math.max(1,Math.round((new Date(sorted[i]+'T00:00:00')-new Date(sorted[i-1]+'T00:00:00'))/86400000)));let avg=gaps.length?Math.round(gaps.reduce((a,b)=>a+b,0)/gaps.length):30;let days=x.last?Math.floor((new Date()-new Date(x.last+'T00:00:00'))/86400000):999;let risk=days>=Math.max(45,avg*2);return{...x,avgDays:avg,days,risk}}).filter(x=>x.risk&&x.total>0).sort((a,b)=>b.total-a.total).slice(0,6);
  const day=new Date().getDate(),lastDay=new Date(new Date().getFullYear(),new Date().getMonth()+1,0).getDate();
  const revenueForecast=day?d.monthRev/day*lastDay:d.monthRev;
  const profitForecast=day?d.net/day*lastDay:d.net;
  const goalForecastPct=d.goal?Math.round(revenueForecast/d.goal*100):0;
  const margin=d.monthRev?Math.round(d.net/d.monthRev*100):0;
  let health=50;
  health+=(d.transactions?15:-10);
  health+=Math.max(-20,Math.min(20,margin));
  health+=d.goal?(goalForecastPct>=100?18:goalForecastPct>=80?8:-8):5;
  health-=Math.min(18,(d.low||[]).length*3);
  health-=Math.min(12,(d.receivables||[]).length*2);
  health-=Math.min(10,atRisk.length*2);
  health=Math.max(0,Math.min(100,Math.round(health)));
  const cls=health>=75?'good':health>=50?'warn':'bad';
  const label=health>=75?'Saúde forte':health>=50?'Saúde em atenção':'Saúde crítica';
  const insights=[];
  insights.push(d.monthRev?`Previsão de faturamento do mês: ${money(revenueForecast)}.`:'Sem vendas suficientes para prever faturamento.');
  if(d.goal)insights.push(goalForecastPct>=100?`A tendência atual passa da meta mensal em ${goalForecastPct}%.`:`A tendência atual fecha em ${goalForecastPct}% da meta.`);
  if(stockForecast[0])insights.push(`${stockForecast[0].p.name} precisa de atenção: cobertura ${stockForecast[0].daysLeft===999?'sem giro':stockForecast[0].daysLeft+' dia(s)'}.`);
  if(rising[0])insights.push(`${rising[0].p.name} está em alta: +${Math.round(rising[0].pct)}% nos últimos 30 dias.`);
  if(falling[0])insights.push(`${falling[0].p.name} caiu ${Math.abs(Math.round(falling[0].pct))}% nos últimos 30 dias.`);
  if(atRisk[0])insights.push(`${atRisk[0].c.name} é cliente em risco: ${atRisk[0].days} dias sem comprar.`);
  return{stockForecast,rising:rising.slice(0,5),falling:falling.slice(0,5),atRisk,revenueForecast,profitForecast,goalForecastPct,health,healthCls:cls,healthLabel:label,insights};
}
function dashboardPredictivePanel(d){
  const p=dashboardPredictiveData(d);
  return `<div class="panel predictive-panel"><div class="between"><div><h3>🔮 Inteligência preditiva</h3><p class="muted small">Previsões de estoque, faturamento, produtos em alta/queda e clientes em risco.</p></div></div><div class="grid cards predictive-cards"><div class="card"><span>Previsão de faturamento</span><b>${money(p.revenueForecast)}</b><small>${d.goal?`${p.goalForecastPct}% da meta mensal`:'Baseado no ritmo atual'}</small></div><div class="card"><span>Previsão de lucro</span><b class="${p.profitForecast>=0?'good':'bad'}">${money(p.profitForecast)}</b><small>Estimativa até o fim do mês</small></div><div class="card"><span>Saúde comercial</span><b class="${p.healthCls}">${p.health}%</b><small>${p.healthLabel}</small></div><div class="card"><span>Clientes em risco</span><b class="${p.atRisk.length?'warn':'good'}">${p.atRisk.length}</b><small>Compravam e ficaram parados</small></div></div><div class="smart-score-track predictive-track"><i style="width:${p.health}%"></i></div><div class="grid two"><div><h4>Previsão de estoque</h4>${p.stockForecast.length?table(['Produto','Estoque','Cobertura','Comprar'],p.stockForecast.map(x=>[esc(x.p.name),`${x.stock} / mín. ${x.min}`,x.daysLeft===999?'Sem giro':`${x.daysLeft} dia(s)`,x.suggested?`<b>${x.suggested} un</b>`:'-'])):'<p class="muted">Nenhum risco de estoque previsto agora.</p>'}</div><div><h4>Centro de insights</h4><div class="insight-list">${p.insights.map(i=>`<button data-dash-go="reports"><b>💡</b><span>${esc(i)}</span></button>`).join('')}</div></div></div><div class="grid three"><div><h4>Produtos em alta</h4>${p.rising.length?table(['Produto','Variação','Vendido'],p.rising.map(x=>[esc(x.p.name),`<b class="good">+${Math.round(x.pct)}%</b>`,`${x.cur} un`])):'<p class="muted">Sem alta relevante nos últimos 30 dias.</p>'}</div><div><h4>Produtos em queda</h4>${p.falling.length?table(['Produto','Variação','Antes'],p.falling.map(x=>[esc(x.p.name),`<b class="bad">${Math.round(x.pct)}%</b>`,`${x.prev} un`])):'<p class="muted">Sem queda relevante nos últimos 30 dias.</p>'}</div><div><h4>Clientes em risco</h4>${p.atRisk.length?table(['Cliente','Sem comprar','Histórico'],p.atRisk.map(x=>[esc(x.c.name),`${x.days} dias`,money(x.total)])):'<p class="muted">Nenhum cliente importante em risco.</p>'}</div></div></div>`;
}

const views={dashboard(){let d=dashboardData(),profitClass=d.net>=0?'good':'bad',cashTotal=sum(d.openCash,'expectedAmount')+sum(d.openCash,'initialAmount');return `<div class="dashboard-pro dashboard-pro-720"><div class="dash-hero panel"><div><h2>Bom dia, ${esc(db.session?.name||'Usuário')} 👋</h2><p class="muted">Painel inteligente com vendas, lucro, Cloud, sincronização, backup e alertas comerciais.</p></div><div class="dash-period"><span>${esc(d.periodLabel)}</span><span>Hoje: ${money(d.today)}</span><b>Meta ${d.goalPct}%</b></div></div>${dashboardPeriodBar(d)}${dashboardExecutiveSummary(d)}<div class="grid cards dash-pro-cards">${dashboardKpi('Faturamento hoje',money(d.today),trendBadge(d.today,d.todayPrev),'good','pdv')}${dashboardKpi('Lucro líquido',money(d.net),trendBadge(d.net,d.prevNet),profitClass,'finance')}${dashboardKpi('Ticket médio',money(d.ticket),trendBadge(d.ticket,d.prevTicket),'','reports')}${dashboardKpi('Meta mensal',`${d.goalPct}%`,`<div class="progress"><i style="width:${d.goalPct}%"></i></div><small>${money(d.monthRev)} de ${money(d.goal)}</small>`,'warn','settings')}${dashboardKpi('Vendas no período',String(d.transactions),trendBadge(d.transactions,d.prevTransactions),'','reports')}</div><div class="grid dash-main"><div class="panel dash-chart-card wide"><div class="between"><h3>Faturamento diário</h3><span class="legend"><i class="current"></i> Este período <i class="previous"></i> Semana anterior</span></div>${dashboardLineChart(d.days,['current','previous'])}</div><div class="panel dash-chart-card"><h3>Formas de pagamento</h3>${dashboardDonut(d.payments,d.monthRev,'Formas de pagamento')}</div></div><div class="grid dash-main"><div class="panel dash-chart-card"><h3>Faturamento por categoria</h3>${dashboardDonut(d.catRows,d.monthRev,'Categorias')}</div><div class="panel dash-chart-card wide"><div class="between"><h3>Comparativo mensal</h3><span class="legend"><i class="current"></i> Atual <i class="previous"></i> Anterior</span></div>${dashboardBarCompare(d.months)}</div></div><div class="grid dash-main"><div class="panel dash-chart-card wide"><h3>Vendas por horário</h3>${dashboardLineChart(d.hours.map(h=>({label:h.label,current:h.value,previous:0})),['current'])}</div><div class="panel"><h3>Top 5 produtos mais vendidos</h3><div class="ranking-list">${d.top.map((x,i)=>`<button data-dash-go="products"><b>${['🥇','🥈','🥉','4º','5º'][i]}</b><span>${esc(x.p.name)}</span><em>${x.qty} un</em><small>${money(x.total)}</small></button>`).join('')||'<p class="muted">Sem vendas para calcular ranking.</p>'}</div></div></div><div class="grid three"><div class="panel"><h3>Metas e tendências</h3><div class="goal-box dash-goal-pro"><b>${d.goalPct}%</b><div class="progress"><i style="width:${d.goalPct}%"></i></div><span>${money(d.monthRev)} de ${money(d.goal)}</span><small>Projeção do mês: <strong>${money(d.projection)}</strong></small><small>Necessário/dia: <strong>${money(d.needPerDay)}</strong></small></div></div><div class="panel"><h3>Alertas inteligentes</h3>${dashboardAlertRows(d)}</div><div class="panel"><h3>Resumo do caixa</h3><div class="health-list"><button data-dash-go="pdv"><span>Caixas abertos</span><b>${d.openCash.length}</b></button><button data-dash-go="pdv"><span>Saldo em caixas</span><b>${money(cashTotal)}</b></button><button data-dash-go="stock"><span>Produtos críticos</span><b>${d.low.length}</b></button><button data-dash-go="finance"><span>Contas em aberto</span><b>${money(sum(d.receivables,'value'))}</b></button></div></div></div>${dashboardCommercialIntelligence(d)}<div class="grid two">${dashboardSmartScore(d)}${dashboardActionCenter(d)}</div>${dashboardPredictivePanel(d)}${dashboardBiProPanel(d)}${dashboardReorderPlan(d)}${dashboardOpportunityPanel(d)}<div class="grid two">${dashboardTopClients(d)}${dashboardUpcoming(d)}</div><div class="grid two"><div class="panel"><h3>Atividades recentes</h3>${table(['Data','Usuário','Ação'],(db.audit||[]).slice(0,6).map(a=>[br(a.date),esc(a.user),esc(a.action)]))}</div><div class="panel"><h3>Saúde do negócio</h3><div class="health-list"><div><span>Faturamento do período</span><b>${money(d.monthRev)}</b></div><div><span>Custo dos produtos</span><b>${money(d.cost)}</b></div><div><span>Despesas</span><b>${money(d.exp)}</b></div><div><span>Lucro líquido</span><b class="${profitClass}">${money(d.net)}</b></div></div></div></div><div class="grid two">${dashboardRecentSales(d)}${dashboardStockRisk(d)}</div>${dashboardQuickActions()}</div>`},
cloud(){return cloudCenterView()},
...
function commercialReadiness(){
  const items=[];
  const add=(ok,icon,title,msg)=>items.push({ok,icon,title,msg});
  add(!!(db.settings.company&&db.settings.company!=='Minha Empresa'),'🏢','Dados da empresa','Nome, telefone e dados da empresa revisados.');
  add(!!(db.products||[]).length,'📦','Produtos','Produtos cadastrados para testar PDV, estoque, etiquetas e relatórios.');
  add(!!(db.clients||[]).length,'👥','Clientes','Clientes cadastrados para histórico, orçamentos e financeiro.');
  add(!!(db.suppliers||[]).length,'🏢','Fornecedores','Fornecedores cadastrados para compras e reposição.');
...
function commercialReadinessView(){
  ...
  return `<div class="panel nested commercial-review"><div class="between"><div><h3>Revisão final comercial</h3><p class="muted small">Checklist para testar o NexaGest antes de entregar, vender ou publicar uma nova versão.</p></div><span class="pill ${pct>=80?'good':pct>=50?'warn':'danger'}">${pct}% pronto</span></div><div class="review-checklist">${items.map(i=>`<div class="review-row ${i.ok?'ok':'pending'}"><b>${i.ok?'✅':'⬜'} ${i.icon} ${esc(i.title)}</b><span>${esc(i.msg)}</span></div>`).join('')}</div><div class="row wrap"><button class="ghost" id="exportCommercialChecklist">Exportar checklist</button><button class="ghost" id="openFirstUseWizardReview">Assistente de primeiro uso</button><button class="ghost" id="exportDiagnosticsReview">Exportar diagnóstico</button></div></div>`;
}
function exportCommercialChecklist(){
  const lines=['CHECKLIST COMERCIAL - NEXAGEST','Versão: '+APP_VERSION,'Gerado em: '+new Date().toLocaleString('pt-BR'),''];
  commercialReadiness().forEach(i=>lines.push((i.ok?'[OK] ':'[ ] ')+i.title+' - '+i.msg));
  download('checklist-comercial-nexagest.txt',lines.join('\n'));
  addSystemLog('Checklist comercial exportado.','success');
}
...
function commercialUpdateStatusView(){
  ...
  const repo=(db.settings.githubOwner||'Diogocas')+'/'+(db.settings.githubRepo||'NexaGest');
  const hint='<small>Repositório configurado: '+esc(repo)+' • Para atualização automática, a Release precisa estar pública e conter latest.yml, NexaGest-Setup.exe e .blockmap.</small>';
  if(!info || !Object.keys(info).length)return '<div class="update-status-box neutral"><b>Nenhuma verificação realizada</b><span>Use “Verificar atualização agora” para testar o GitHub Releases.</span>'+hint+'</div>';
  ...
  const details=info.error?esc(info.error):esc(info.notes||info.status||'Nenhuma observação informada.');
...
function addLicenseMinutes(minutes){
...
function licenseMinutesLeft(expiresAt){
  if(!expiresAt)return null;
  const raw=String(expiresAt);
  const exp=raw.includes('T')?new Date(raw):new Date(raw+'T23:59:59');
  if(isNaN(exp))return null;
...
function ensureAutomaticTrialLicense(){
  ...
  db.settings.licenseKey='NEXA-TRIAL-8D-AUTOMATICO';
  db.settings.licenseOwner='Teste gratuito';
  db.settings.licenseEmail='';
  db.settings.licenseType='trial';
  db.settings.licenseDurationDays=AUTO_TRIAL_DAYS;
  db.settings.licenseActivatedAt=start;
  db.settings.licenseExpiresAt=addLicenseDays(start,AUTO_TRIAL_DAYS);
  ...
  addSystemLog('Teste automático de 8 dias criado.','success');
  save();
  return true;
}
function effectiveLicenseState(){
  ensureAutomaticTrialLicense();
  const clean=String(db.settings.licenseKey||'').trim();
  const cleanUpper=clean.toUpperCase();
  if(cleanUpper==='NEXA-TRIAL-8D-AUTOMATICO' || cleanUpper==='NEXA-TRIAL-7D-AUTOMATICO' || db.settings.licenseType==='trial'){
    const left=licenseDaysLeft(db.settings.licenseExpiresAt);
    const expired=Number.isFinite(left)&&left<0;
    return {ok:!expired, validLicense:true, status:expired?'expired':'trial', label:expired?'Teste expirado':'Teste gratuito', type:'trial', durationDays:AUTO_TRIAL_DAYS, activatedAt:db.settings.licenseActivatedAt, expiresAt:db.settings.licenseExpiresAt, daysLeft:left, message:expired?'O teste gratuito expirou. Ative uma licença para continuar.':`Teste gratuito ativo. Restam ${left} dia(s).`};
  }
...
function licenseExpirationDate(expiresAt){
  if(!expiresAt)return null;
  const raw=String(expiresAt);
  const exp=raw.includes('T')?new Date(raw):new Date(raw+'T23:59:59');
  return isNaN(exp)?null:exp;
}
function licenseRemainingMs(state=effectiveLicenseState()){
  const exp=licenseExpirationDate(state.expiresAt||db.settings.licenseExpiresAt);
  if(!exp)return null;
  return exp-new Date();
}
function formatLicenseRemaining(state=effectiveLicenseState()){
  if(state.type==='permanent')return 'Licença permanente';
  const ms=licenseRemainingMs(state);
  if(!Number.isFinite(ms)){
    if(state.durationMinutes)return `${state.durationMinutes} minuto(s) após ativar`;
    if(state.durationDays)return `${state.durationDays} dia(s) após ativar`;
    return '—';
  }
...
function licenseFriendlyMessage(state=effectiveLicenseState()){
  if(state.type==='permanent')return 'Licença permanente ativa. O NexaGest está liberado sem vencimento.';
  if(state.ok===false)return 'A licença expirou. Seus dados estão preservados; ative uma nova licença para continuar.';
  if(state.status==='pending')return state.durationMinutes?`Licença pronta para ativar. A contagem de ${state.durationMinutes} minuto(s) começa na primeira ativação.`:`Licença pronta para ativar. A contagem de ${state.durationDays||30} dia(s) começa na primeira ativação.`;
  return `Licença por período ativa. ${formatLicenseRemaining(state)}.`;
}
function licenseGlobalWarningData(state=effectiveLicenseState()){
  try{
    if(!state || state.ok===false || state.type==='permanent')return null;
    const ms=licenseRemainingMs(state);
    if(!Number.isFinite(ms) || ms<=0)return null;
    const dayMs=86400000;
    if(ms>LICENSE_WARNING_DAYS*dayMs)return null;
    const remaining=formatLicenseRemaining(state).replace(/^Restam?\s?/,'').replace(/^Resta\s?/,'');
    let level='warn';
    let text='⚠️ Licença vence em '+remaining;
    if(ms<=dayMs){ level='danger'; text='🔴 Licença vence hoje'; }
    else if(ms<=2*dayMs){ level='orange'; text='🟠 Licença vence em '+remaining; }
    return {level,text,title:'Aviso de licença: ative uma nova licença antes do vencimento para evitar o bloqueio do NexaGest.'};
  }catch(e){return null}
}
function refreshLicenseCountdownLabels(){
  document.querySelectorAll('[data-license-countdown]').forEach(el=>{
    try{ el.textContent=formatLicenseRemaining(effectiveLicenseState()); }catch(e){}
  });
  document.querySelectorAll('[data-license-message]').forEach(el=>{
    try{ el.textContent=licenseFriendlyMessage(effectiveLicenseState()); }catch(e){}
  });
}
...
function licenseWarningText(state=effectiveLicenseState()){
  ...
  const ms=licenseRemainingMs(state);
  if(Number.isFinite(ms) && ms<=0)return 'Sua licença expirou. Ative uma nova licença para continuar usando o NexaGest.';
  if(Number.isFinite(ms) && ms<=3600000)return `Atenção: sua licença vence em ${formatLicenseRemaining(state).replace(/^Restam?\s?/,'')}. Após o vencimento o NexaGest será bloqueado até a ativação de uma nova licença.`;
  if(Number.isFinite(state.daysLeft) && state.daysLeft<=LICENSE_WARNING_DAYS)return `Atenção: sua licença vence em ${formatLicenseRemaining(state).replace(/^Restam?\s?/,'')}. Após o vencimento o NexaGest será bloqueado até a ativação de uma nova licença.`;
  return '';
}
...
function ensureLicenseGateTimer(){
  ...
  },LICENSE_GATE_INTERVAL_MS);
}
function licenseBlockedView(){
  const state=effectiveLicenseState();
  const exp=formatLicenseDate(state.expiresAt||db.settings.licenseExpiresAt);
  return `<div class="license-block-screen"><div class="login-card license-block-card"><div class="brand-large">${logo()}<div><h1>Licença expirada</h1><p class="muted">Seus dados estão preservados.</p></div></div><div class="notice"><b>🔒 O período de uso terminou.</b><br><span>Para continuar usando o NexaGest, cole a nova licença recebida.</span></div><div class="commercial-grid"><div class="commercial-box"><b>Status</b><span>${esc(state.label||'Expirada')}</span><small>${esc(state.message||'Ative uma nova licença.')}</small></div><div class="commercial-box"><b>Validade anterior</b><span>${esc(exp)}</span><small>O sistema completo será liberado após a ativação.</small></div></div><div class="form-grid"><div class="field wide"><label>Nova licença</label><input id="licenseKey" value="" placeholder="Cole aqui a licença NEXA-..."></div><label class="check-row"><input type="checkbox" id="autoUpdate" ${db.settings.autoUpdate?'checked':''}> Verificar atualizações automaticamente</label><div class="field"><label>&nbsp;</label><button id="saveLicenseSettings" class="ok">Ativar licença</button></div></div><p class="muted small">O cliente e o e-mail são lidos automaticamente da licença gerada no NexaGest License Manager DEV.</p><div class="row wrap"><button class="ghost" id="checkUpdatesNow">Verificar atualização</button><button class="ghost" id="logout">Sair</button></div></div></div>`;
}
function commercialLicensePanel(){
  ...
  const remainingLabel=formatLicenseRemaining(result);
  const friendlyMessage=licenseFriendlyMessage(result);
  ...
  const ownerDisplay=db.settings.licenseOwner||result.owner||'—';
  const emailDisplay=db.settings.licenseEmail||result.email||'—';
  return `<div class="panel settings-card smart-section commercial-panel"><div class="between"><div><h3>Licença e atualização</h3><p class="muted small">Ative o NexaGest colando a licença recebida. Cliente e e-mail são preenchidos automaticamente pela chave.</p></div><span class="pill ${cls}">${esc(result.label||'Status')}</span></div><div class="commercial-grid"><div class="commercial-box license-status-box"><b>Status da licença</b><span>${esc(result.ok===false?'Licença expirada':(result.type==='permanent'?'Licença permanente':'Licença ativa'))}</span><small data-license-message>${esc(friendlyMessage)}</small></div><div class="commercial-box license-countdown-box"><b>Tempo restante</b><span data-license-countdown>${esc(remainingLabel)}</span><small>Tipo: ${esc(licenseType)}</small></div><div class="commercial-box"><b>Cliente</b><span>${esc(ownerDisplay)}</span><small>${esc(emailDisplay)}</small></div><div class="commercial-box"><b>Validade</b><span>${esc(expLabel)}</span><small>Ativada em: ${esc(activatedLabel)}</small></div><div class="commercial-box"><b>Versão instalada</b><span>${esc(APP_VERSION)}</span><small>Última verificação: ${esc(last)}</small></div></div>${licenseWarningText(result)?`<div class="license-warning-box"><b>⚠️ Aviso de licença</b><span>${esc(licenseWarningText(result))}</span></div>`:''}<div class="form-grid"><div class="field wide"><label>Licença</label><input id="licenseKey" value="${esc(db.settings.licenseKey||'')}" placeholder="Cole aqui a licença NEXA-... gerada no app DEV"></div><label class="check-row"><input type="checkbox" id="autoUpdate" ${db.settings.autoUpdate?'checked':''}> Verificar atualizações automaticamente</label><div class="field"><label>&nbsp;</label><button id="saveLicenseSettings">Ativar licença</button></div></div><div class="update-status-box neutral"><b>Atualizações</b><span>${esc(updateLabel)}</span><small>As funções de geração de licença ficam somente no NexaGest License Manager DEV.</small></div><div class="row wrap"><button class="ghost" id="checkUpdatesNow">Verificar atualização agora</button><button class="ghost" id="downloadAvailableUpdate">Baixar/instalar atualização</button><button class="ghost" id="openFirstUseWizard">Assistente de primeiro uso</button></div></div>`
}
function saveLicenseSettings(){
  ...
  const ownerInput=document.getElementById('licenseOwner');
  const emailInput=document.getElementById('licenseEmail');
  if(ownerInput)db.settings.licenseOwner=ownerInput.value.trim();
  if(emailInput)db.settings.licenseEmail=emailInput.value.trim();
  db.settings.autoUpdate=!!document.getElementById('autoUpdate')?.checked;
  const cleanKey=window.NexaGestPremium?.normalizeKey?window.NexaGestPremium.normalizeKey(db.settings.licenseKey):String(db.settings.licenseKey||'').trim();
  const cleanKeyUpper=cleanKey.toUpperCase();
  if(cleanKeyUpper==='NEXA-TRIAL-8D-AUTOMATICO' || cleanKeyUpper==='NEXA-TRIAL-7D-AUTOMATICO'){
  ...
  if(result.validLicense){
    ...
    db.settings.licenseDurationMinutes=result.durationMinutes||'';
    ...
    if(result.type==='period'){
      if(result.needsActivation || db.settings.licenseActivationKey!==cleanKey || !db.settings.licenseActivatedAt || !db.settings.licenseExpiresAt){
        ...
        db.settings.licenseExpiresAt=result.durationMinutes?addLicenseMinutes(result.durationMinutes):addLicenseDays(db.settings.licenseActivatedAt,result.durationDays||30);
...
async function activateOnlineLicense(){
  ...
  if(r?.token)db.settings.licenseToken=r.token;
  db.settings.licenseStatus=r?.status||db.settings.licenseStatus;
  db.settings.licenseOnlineInfo=r||{};
  save();
  addSystemLog('Licença online: '+(r?.ok?'ativada':'falhou'),r?.ok?'success':'warn');
  alert(r?.ok?'Licença online ativada com sucesso.':'Não foi possível ativar online: '+(r?.error||r?.message||'verifique a URL do servidor.'));
  app();
}
async function validateOnlineLicense(){
  saveLicenseSettingsNoAlert();
  const payload={licenseServerUrl:db.settings.licenseServerUrl,licenseKey:db.settings.licenseKey,licenseToken:db.settings.licenseToken,licenseEmail:db.settings.licenseEmail};
  const r=await window.nexagest?.licenseOnlineValidate?.(payload);
  db.settings.licenseLastOnlineCheck=new Date().toISOString();
  db.settings.licenseDeviceId=r?.deviceId||db.settings.licenseDeviceId||'';
  db.settings.licenseStatus=r?.status||db.settings.licenseStatus;
  db.settings.licenseOnlineInfo=r||{};
  save();
  addSystemLog('Licença online validada: '+(r?.ok?'OK':'aviso'),r?.ok?'success':'warn');
  alert(r?.ok?'Licença validada online.':'Validação online não concluída: '+(r?.error||r?.message||'servidor indisponível.'));
  app();
}
...
async function checkUpdatesNow(){
  ...
  try{
    ...
    if(r?.updateAvailable){
      showNexaGestUpdateModal({...r, channel:'prompt-download'});
    }else if(r?.ok===false){
...
function updateDiagnosticsText(){
  const info=db.settings.lastUpdateInfo||{};
  const lines=[
    'NexaGest - Diagnóstico de atualização',
    'Versão instalada: '+APP_VERSION,
    'GitHub: '+(db.settings.githubOwner||'Diogocas')+'/'+(db.settings.githubRepo||'NexaGest'),
    'Auto update: '+(db.settings.autoUpdate?'ligado':'desligado'),
    'Última verificação: '+(db.settings.lastUpdateCheckAt||'nunca'),
    'Status: '+(info.status||info.source||'sem verificação'),
    'Última versão detectada: '+(info.latestVersion||info.version||'—'),
    'Atualização disponível: '+(info.updateAvailable?'sim':'não'),
    'Erro: '+(info.error||'—')
  ];
  return lines.join('\n');
}
...
function cloudCenterMetrics(){
  const provider=db.settings.backupProvider||'Local';
  const files=cloudBackupFiles();
  const history=cloudBackupHistory();
  const role=db.settings.networkRole||'standalone';
  const serverRunning=!!db.settings.networkServerRunning;
  const syncOn=role!=='standalone'||!!db.settings.cloudSyncEnabled;
  const multiPc=role==='server'||role==='client'||!!db.settings.cloudMultiPcEnabled;
  const lastBackup=db.settings.cloudLastBackupAt||history[0]?.date||localStorage.getItem('nexagest-last-auto-cloud-backup')||'';
  const lastSync=db.settings.cloudLastSyncAt||db.settings.lastSyncAt||db.settings.lastBusinessSyncAt||db.settings.lastCatalogSyncAt||'';
  const backupOk=!!lastBackup||files.length>0;
  const connected=serverRunning||db.settings.networkLastTestOk;
  let score=25;
  if(provider&&provider!=='Local')score+=20;
  if(backupOk)score+=20;
  if(syncOn)score+=15;
  if(multiPc)score+=10;
  if(connected)score+=10;
  score=Math.max(0,Math.min(100,score));
  return{provider,files,history,role,serverRunning,syncOn,multiPc,lastBackup,lastSync,backupOk,connected,score};
}
function cloudStatusPill(m){
  if(m.score>=80)return '<span class="pill good">Cloud saudável</span>';
  if(m.score>=55)return '<span class="pill warn">Cloud em implantação</span>';
  return '<span class="pill danger">Cloud precisa configurar</span>';
}
function cloudCenterView(){
  const m=cloudCenterMetrics();
  const roleLabel={standalone:'Individual',server:'Servidor principal',client:'Cliente / terminal'}[m.role]||m.role;
  const serverUrl=db.settings.networkServerUrl||db.settings.serverAddress||('http://127.0.0.1:'+(db.settings.networkPort||3333));
  const backupPanel=cloudBackupSettingsView();
  const networkPanel=networkSettingsPanel();
  const devices=[
    {name:db.settings.cloudDeviceName||'Este computador',type:roleLabel,status:m.connected?'Online / preparado':'Aguardando conexão',last:m.lastSync?br(m.lastSync):'Sem sincronização'},
    ...(m.role==='server'?[{name:'Terminais conectados',type:'Multi-PC',status:String(db.settings.networkClientsConnected||0)+' conectado(s)',last:db.settings.networkLastTestAt?br(db.settings.networkLastTestAt):'—'}]:[]),
    ...(m.role==='client'?[{name:'Servidor NexaGest',type:'Origem dos dados',status:db.settings.networkLastTestOk?'Respondendo':'Não testado',last:db.settings.networkLastTestAt?br(db.settings.networkLastTestAt):'—'}]:[])
  ];
  return `<div class="cloud-page"><div class="dash-hero panel cloud-hero"><div><h2>☁️ NexaGest Cloud</h2><p class="muted">Sincronização, backup online, backup exportável e estrutura multi computador.</p></div><div class="cloud-score"><b>${m.score}%</b>${cloudStatusPill(m)}</div></div><div class="grid cards cloud-cards"><div class="card"><span>Sincronização</span><b>${m.syncOn?'Ativa / preparada':'Manual'}</b><small>${m.lastSync?br(m.lastSync):'Ainda não sincronizado'}</small></div><div class="card"><span>Backup online</span><b>${esc(m.provider)}</b><small>${m.lastBackup?br(m.lastBackup):'Nenhum backup recente'}</small></div><div class="card"><span>Multi computador</span><b>${m.multiPc?'Preparado':'Desligado'}</b><small>${esc(roleLabel)}</small></div><div class="card"><span>Servidor</span><b>${m.serverRunning?'Ligado':'Parado'}</b><small>${esc(serverUrl)}</small></div></div><div class="grid two"><div class="panel cloud-action-panel"><div class="between"><div><h3>Operação rápida</h3><p class="muted small">Ações principais para manter dados protegidos e computadores alinhados.</p></div></div><div class="cloud-actions"><button class="ok" id="runCloudBackup">Gerar backup Cloud agora</button><button class="ghost" id="refreshCloudBackups">Atualizar lista de backups</button><button class="ghost" id="restoreCloudBackup">Restaurar backup Cloud</button>${m.role==='server'?(m.serverRunning?'<button class="danger" id="stopNetworkServer">Parar servidor multi-PC</button>':'<button class="ok" id="startNetworkServer">Iniciar servidor multi-PC</button>'):''}${m.role==='client'?'<button class="ok" id="syncCatalogData">Sincronizar cadastros</button><button class="ok" id="syncBusinessData">Sincronizar vendas/caixa</button><button class="ghost" id="testNetworkClient">Testar conexão</button>':''}<button class="ghost" id="networkRealtimeCheck">Checar tempo real</button></div><div class="cloud-checklist"><b>Checklist Cloud</b><span class="${m.provider!=='Local'?'good':'warn'}">${m.provider!=='Local'?'✓':'•'} Provedor/pasta sincronizada: ${esc(m.provider)}</span><span class="${m.backupOk?'good':'warn'}">${m.backupOk?'✓':'•'} Backup criado ou localizado</span><span class="${m.multiPc?'good':'warn'}">${m.multiPc?'✓':'•'} Modo multi-PC configurado</span><span class="${m.connected?'good':'warn'}">${m.connected?'✓':'•'} Comunicação testada</span></div></div><div class="panel"><div class="between"><h3>Computadores / terminais</h3><span class="pill">${devices.length}</span></div>${table(['Nome','Tipo','Status','Última atividade'],devices.map(d=>[esc(d.name),esc(d.type),esc(d.status),esc(d.last)]))}<p class="muted small">Para usar em mais de um computador: defina um PC como Servidor e os outros como Cliente usando o endereço do servidor.</p></div></div><div class="grid two"><div class="panel cloud-settings-panel"><h3>Backup Cloud</h3>${backupPanel}</div><div class="panel cloud-settings-panel"><h3>Sincronização e Multi-PC</h3>${networkPanel}</div></div><div class="panel"><div class="between"><h3>Histórico Cloud</h3><span class="pill">Backups e sincronizações</span></div><div class="grid two"><div>${cloudBackupHistoryView()}</div><div>${networkLogHtml()}</div></div></div></div>`;
}
function markCloudBackupSuccess(){db.settings.cloudLastBackupAt=new Date().toISOString();db.settings.cloudLastStatus='Backup atualizado';}
function markCloudSyncSuccess(){db.settings.cloudLastSyncAt=new Date().toISOString();db.settings.cloudLastStatus='Sincronização atualizada';}
...
function networkSettingsPanel(){
  ...
  return `<div class="panel settings-card smart-section network-panel network-v5011"><div class="between"><div><h3>Sincronização real entre computadores</h3><p class="muted small">Atualização em tempo real, monitoramento automático e conflitos básicos.</p></div><span class="pill ${info.cls}">${info.icon} ${esc(info.label)}<small id="networkTopUptimeLive" class="network-pill-extra"></small>${topExtra}</span></div>
  ...
  <div class="network-note"><b>Sincronização:</b><span>Operação com monitoramento em tempo real e conflitos básicos. O cliente verifica alterações do servidor automaticamente.</span></div>
...
function applyCatalogSnapshot(catalog){
  ...
  db.settings.cloudLastSyncAt=db.settings.lastSyncAt;
...
function financeEnterpriseFinalCockpit(f){
  ...
  return `<div class="panel finance-final-cockpit"><div class="between"><div><h3>Central Financeira</h3><p class="muted small">Resumo final dos recursos financeiros ativos nesta versão.</p></div><span class="pill ${health}">${healthText}</span></div><div class="finance-final-grid"><div class="finance-final-score ${health}"><span>Saldo projetado</span><b>${money(projected)}</b><small>Saldo atual + receber - pagar</small></div>${checks.map(c=>`<div class="finance-final-check ${c[2]}"><b>${esc(c[0])}</b><span>${esc(c[1])}</span></div>`).join('')}</div></div>`;
}
function financeMiniMetric(label,value,cls,small){return `<div class="finance-clean-mini"><span>${label}</span><b class="${cls||''}">${value}</b><small>${small||''}</small></div>`}
...
function financeCostCenterPanel(f){
  ...
  return `<div class="panel finance-cost-center-panel"><div class="between"><div><h3>Centro de custos</h3><p class="muted small">Saídas do mês separadas por área.</p></div><span class="pill bad">${money(total)}</span></div><div class="finance-cost-centers">${rows.slice(0,8).map(x=>`<div class="finance-cost-row"><div class="between"><b>${esc(x.name)}</b><span>${money(x.value)}</span></div><div class="finance-clean-track"><i style="width:${Math.max(6,Number(x.value||0)/max*100)}%"></i></div><small>${Math.round((Number(x.value||0)/(total||1))*100)}% das saídas</small></div>`).join('')}</div></div>`;
}
...
/* Contas a pagar: filtros rápidos seguros */
...
/* OFX: filtros, classificação e análise bancária segura */
...
function cleanUpdateNotesText(notes){
  ...
  text=text.split('\n')
    ...
    .filter(Boolean)
    .filter(x=>!/^nexagest$/i.test(x))
    .filter(x=>!/^sistema profissional de gestão comercial/i.test(x))
    .join('\n');
  return text.trim();
}
function updateNotesHtml(notes){
  const text=cleanUpdateNotesText(notes);
  if(!text){
    return '<li>Atualizador integrado ao NexaGest.</li><li>Correções e melhorias de estabilidade.</li><li>Instalação segura via GitHub Releases.</li>';
  }
  return text.split(/\n+/)
    .map(x=>x.trim())
    .filter(Boolean)
    .slice(0,8)
    .map(x=>`<li>${esc(x)}</li>`)
    .join('') || '<li>Melhorias e correções da nova versão.</li>';
}
function showNexaGestUpdateModal(payload={}){
  nexagestUpdateState={...nexagestUpdateState,...payload};
  const version=esc(nexagestUpdateState.latestVersion||nexagestUpdateState.version||'nova');
  const progress=Math.max(0,Math.min(100,Number(nexagestUpdateState.progress||0)));
  const downloaded=!!nexagestUpdateState.downloaded || nexagestUpdateState.channel==='prompt-install' || nexagestUpdateState.channel==='downloaded';
  const downloading=String(nexagestUpdateState.channel||'').includes('progress') || String(nexagestUpdateState.status||'').toLowerCase().includes('baixando');
  const error=nexagestUpdateState.ok===false;
  const status=esc(nexagestUpdateState.status||'Nova versão disponível');
  const transfer=nexagestUpdateState.total?`${formatBytes(nexagestUpdateState.transferred)} de ${formatBytes(nexagestUpdateState.total)}`:'';
  const speed=nexagestUpdateState.bytesPerSecond?` • ${formatBytes(nexagestUpdateState.bytesPerSecond)}/s`:'';
  let actions='';
  if(error){
    actions='<button class="ghost" id="ngUpdaterClose">Fechar</button><button id="ngUpdaterCheckAgain">Tentar novamente</button>';
  }else if(downloaded){
    actions='<button class="ghost" id="ngUpdaterLater">Instalar ao sair</button><button id="ngUpdaterInstall">Reiniciar e instalar</button>';
  }else if(downloading){
    actions='<button class="ghost" id="ngUpdaterLater">Continuar em segundo plano</button>';
  }else{
    actions='<button class="ghost" id="ngUpdaterLater">Depois</button><button id="ngUpdaterDownload">Atualizar agora</button>';
  }
  const html=`<div class="modal-overlay show updater-pro-overlay" id="nexagestUpdaterModal"><div class="modal updater-pro-modal"><div class="updater-pro-hero"><div><span class="pill good">Atualização</span><h2>🚀 NexaGest ${version} disponível</h2><p>${status}</p></div><button class="ghost" id="ngUpdaterX">Esc</button></div><div class="updater-pro-grid"><div><span>Versão instalada</span><b>${esc(APP_VERSION)}</b></div><div><span>Nova versão</span><b>${version}</b></div><div><span>Origem</span><b>GitHub Releases</b></div></div><div class="updater-pro-progress"><div class="between"><b>${downloaded?'Pronto para instalar':(downloading?'Baixando atualização':'Atualização disponível')}</b><span>${downloaded?'100%':Math.round(progress)+'%'}</span></div><div class="progress"><i style="width:${downloaded?100:progress}%"></i></div><small>${transfer}${speed}</small></div><div class="updater-pro-notes"><b>Novidades</b><ul>${updateNotesHtml(nexagestUpdateState.notes)}</ul></div>${error?`<div class="update-status-box bad"><b>Falha na atualização</b><span>${esc(nexagestUpdateState.error||'Não foi possível concluir a operação.')}</span></div>`:''}<div class="modal-actions updater-pro-actions">${actions}</div></div></div>`;
  document.getElementById('nexagestUpdaterModal')?.remove();
  document.body.insertAdjacentHTML('beforeend',html);
  const close=()=>document.getElementById('nexagestUpdaterModal')?.remove();
  document.getElementById('ngUpdaterX')?.addEventListener('click',close);
  document.getElementById('ngUpdaterClose')?.addEventListener('click',close);
  document.getElementById('ngUpdaterLater')?.addEventListener('click',close);
  document.getElementById('ngUpdaterDownload')?.addEventListener('click',async()=>{document.getElementById('ngUpdaterDownload').disabled=true;await downloadAvailableUpdate();});
  document.getElementById('ngUpdaterInstall')?.addEventListener('click',async()=>{await window.nexagest?.commercialInstallUpdate?.();});
  document.getElementById('ngUpdaterCheckAgain')?.addEventListener('click',async()=>{close();await checkUpdatesNow();});
}
function setupNexaGestUpdaterPro(){
  if(!window.nexagest?.onUpdaterEvent || window.__nexagestUpdaterProReady)return;
  window.__nexagestUpdaterProReady=true;
  window.nexagest.onUpdaterEvent(payload=>{
    db.settings.lastUpdateInfo={...(db.settings.lastUpdateInfo||{}),...payload};
    if(payload?.checkedAt)db.settings.lastUpdateCheckAt=payload.checkedAt;
    try{save()}catch(_){ }
    if(['prompt-download','available','progress','download-started','downloaded','prompt-install','error'].includes(payload?.channel)){
      showNexaGestUpdateModal(payload);
    }
  });
}
setupNexaGestUpdaterPro();

const bindBeforeOfxEnterprise = bind;
bind = function(){bindBeforeOfxEnterprise();bindOfxEnterpriseEvents()};


/* IA integrada ao banco de dados local */
function aiEnsureStore(){
  db.aiConversations=db.aiConversations||[];
  if(!db.aiConversations.length){
    db.aiConversations.push({id:uid(),title:'Conversa inicial',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),messages:[{role:'assistant',text:'Olá! Eu sou o NexaGest AI. Agora estou integrado ao banco de dados local e consigo consultar vendas, produtos, estoque, clientes e financeiro.',date:new Date().toISOString()}]});
  }
  db.aiActiveConversationId=db.aiActiveConversationId||db.aiConversations[0].id;
  return db.aiConversations;
}
function aiActiveConversation(){aiEnsureStore();return db.aiConversations.find(c=>c.id===db.aiActiveConversationId)||db.aiConversations[0]}
function aiQuickQuestions(){return ['Quanto vendi hoje?','Resumo financeiro do mês','Quais produtos estão com estoque crítico?','O que preciso comprar?','Quem são meus clientes VIP?','Quais produtos estão parados?','Quais produtos mais vendidos?','Como está meu contas a receber?','Mostre as últimas vendas','Como está a saúde do negócio?']}
function aiAsArray(v){return Array.isArray(v)?v:[]}
function aiDateOnly(v){return String(v||'').slice(0,10)}
function aiMonthOnly(v){return String(v||'').slice(0,7)}
function aiDaysBetween(date){let t=new Date(aiDateOnly(date)||date||0).getTime();if(!t)return 9999;return Math.floor((Date.now()-t)/86400000)}
function aiProductName(id){let p=product(id);return p?.name||'Produto não encontrado'}
function aiClientName(id){if(!id||id==='balcao')return 'Consumidor final';let c=client(id);return c?.name||'Cliente não encontrado'}
function aiValidSales(){return salesValid? salesValid():aiAsArray(db.sales).filter(s=>!s.cancelled)}
function aiSaleItems(sales){let out=[];(sales||[]).forEach(s=>(s.items||[]).forEach(i=>out.push({...i,sale:s})));return out}
function aiTopProducts(sales,limit=5){let map={};aiSaleItems(sales).forEach(i=>{let id=i.productId||i.id||i.product_id;if(!id)return;let qty=Number(i.qty||i.quantity||i.quantidade||1),total=Number(i.total||0);if(!map[id])map[id]={id,name:aiProductName(id),qty:0,total:0};map[id].qty+=qty;map[id].total+=total});return Object.values(map).sort((a,b)=>b.total-a.total).slice(0,limit)}
function aiTopClients(sales,limit=5){let map={};(sales||[]).forEach(s=>{let id=s.clientId||'balcao';if(!map[id])map[id]={id,name:aiClientName(id),total:0,count:0,last:s.date};map[id].total+=Number(s.total||0);map[id].count++;if(String(s.date||'')>String(map[id].last||''))map[id].last=s.date});return Object.values(map).sort((a,b)=>b.total-a.total).slice(0,limit)}
function aiLowStock(){return aiAsArray(db.products).filter(p=>p.active!==false&&Number(p.stock||0)<=Number(p.min||0)).sort((a,b)=>Number(a.stock||0)-Number(b.stock||0))}
function aiStoppedProducts(days=90){let sales=aiValidSales(),last={};aiSaleItems(sales).forEach(i=>{let id=i.productId||i.id||i.product_id;if(id&&(!last[id]||String(i.sale.date)>String(last[id])))last[id]=i.sale.date});return aiAsArray(db.products).filter(p=>p.active!==false&&Number(p.stock||0)>0&&(!last[p.id]||aiDaysBetween(last[p.id])>=days)).map(p=>({...p,lastSale:last[p.id]||null,daysWithoutSale:last[p.id]?aiDaysBetween(last[p.id]):9999})).sort((a,b)=>b.daysWithoutSale-a.daysWithoutSale)}
function aiFinancialSnapshot(){let t=today(),m=t.slice(0,7);let expenses=aiAsArray(db.expenses),receivables=aiAsArray(db.receivables);let expMonth=expenses.filter(e=>aiMonthOnly(e.date)===m);let recOpen=receivables.filter(r=>!r.paid);let recOverdue=recOpen.filter(r=>aiDateOnly(r.dueDate||r.date)<t);return{expensesMonth:sum(expMonth,'value'),expensesCount:expMonth.length,receivableOpen:sum(recOpen,'value'),receivableCount:recOpen.length,receivableOverdue:sum(recOverdue,'value'),receivableOverdueCount:recOverdue.length,paidReceivables:sum(receivables.filter(r=>r.paid&&aiMonthOnly(r.paidAt||r.date)===m),'value')}}
function aiBusinessSnapshot(){
  let d=dashboardData?dashboardData():stats(),sales=aiValidSales(),t=today(),m=t.slice(0,7);
  let todaySales=sales.filter(s=>aiDateOnly(s.date)===t),monthSales=sales.filter(s=>aiMonthOnly(s.date)===m);
  let low=aiLowStock(),stopped=aiStoppedProducts(90),topProducts=aiTopProducts(monthSales.length?monthSales:sales,5),topClients=aiTopClients(sales,5),fin=aiFinancialSnapshot();
  let cost=sum(monthSales,'cost'),monthRev=sum(monthSales,'total'),gross=monthRev-cost,net=gross-fin.expensesMonth;
  return{today:d.today||sum(todaySales,'total'),todayCount:todaySales.length,month:d.monthRev||monthRev,monthCount:monthSales.length,cost,gross,net,ticket:monthSales.length?monthRev/monthSales.length:(d.ticket||0),low,lowCount:low.length,stopped,stoppedCount:stopped.length,topProducts,topClients,products:aiAsArray(db.products).length,clients:aiAsArray(db.clients).length,sales:sales.length,finance:fin,lastSales:sales.slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).slice(0,5)}
}
function aiList(rows,formatter,empty='Nenhum registro encontrado.'){return rows&&rows.length?rows.map((x,i)=>`${i+1}. ${formatter(x)}`).join('\n'):empty}
function aiReorderPlan(s){let sales30=aiValidSales().filter(x=>aiDaysBetween(x.date)<=30),items=aiSaleItems(sales30),sold={};items.forEach(i=>{let id=i.productId||i.id||i.product_id;if(id)sold[id]=(sold[id]||0)+Number(i.qty||i.quantity||1)});let rows=s.low.map(p=>{let avg=(sold[p.id]||0)/30;let target=Math.max(Number(p.min||0)*2,Math.ceil(avg*15));let buy=Math.max(0,Math.ceil(target-Number(p.stock||0)));return{p,avg,buy}}).filter(x=>x.buy>0).sort((a,b)=>b.buy-a.buy).slice(0,8);return rows}
function aiHealthScore(s){let score=100;if(s.lowCount)score-=Math.min(25,s.lowCount*3);if(s.stoppedCount)score-=Math.min(20,s.stoppedCount*2);if(s.net<0)score-=25;if(s.finance.receivableOverdue>0)score-=15;if(s.monthCount===0)score-=15;return Math.max(0,Math.min(100,Math.round(score)))}
function aiLocalAnswer(text){
  let q=String(text||'').toLowerCase(),s=aiBusinessSnapshot();
  if(q.includes('últimas vendas')||q.includes('ultimas vendas'))return `Consultei o banco de vendas.\n\n${aiList(s.lastSales,x=>`${br(x.date)} • ${aiClientName(x.clientId)} • ${money(x.total)} • ${esc(x.payment||'Pagamento não informado')}`)}`;
  if(q.includes('hoje')||q.includes('vendi'))return `Consultei as vendas de hoje no banco de dados.\n\nVendas de hoje: ${money(s.today)}\nQuantidade de vendas: ${s.todayCount}\nTicket médio do mês: ${money(s.ticket)}\n\n${s.todayCount?'O movimento de hoje já está registrado no histórico de vendas.':'Ainda não encontrei vendas registradas para hoje.'}`;
  if(q.includes('financeiro')||q.includes('contas')||q.includes('receber')||q.includes('pagar'))return `Resumo financeiro consultado no banco:\n\nDespesas do mês: ${money(s.finance.expensesMonth)} (${s.finance.expensesCount} lançamento(s))\nContas a receber em aberto: ${money(s.finance.receivableOpen)} (${s.finance.receivableCount})\nVencidas/em atraso: ${money(s.finance.receivableOverdue)} (${s.finance.receivableOverdueCount})\nLucro líquido estimado do mês: ${money(s.net)}`;
  if(q.includes('estoque')||q.includes('crítico')||q.includes('critico'))return s.lowCount?`Encontrei ${s.lowCount} produto(s) com estoque crítico no banco de produtos.\n\n${aiList(s.low.slice(0,8),p=>`${p.name} — estoque ${Number(p.stock||0)} / mínimo ${Number(p.min||0)}`)}\n\nRecomendo priorizar estes itens antes de novas vendas.`:'Nenhum produto ativo está abaixo ou igual ao estoque mínimo neste momento.';
  if(q.includes('comprar')||q.includes('compra')){let rows=aiReorderPlan(s);return rows.length?`Plano de compras sugerido com base em estoque mínimo e vendas recentes:\n\n${aiList(rows,x=>`${x.p.name} — comprar aprox. ${x.buy} un. (estoque atual ${Number(x.p.stock||0)}, média ${x.avg.toFixed(1)} un/dia)`)}\n\nEsta sugestão usa dados locais de produtos, estoque e vendas dos últimos 30 dias.`:'Não encontrei necessidade clara de compra agora. Nenhum item crítico teve cálculo de reposição relevante com os dados atuais.'}
  if(q.includes('cliente')||q.includes('vip'))return `Clientes VIP identificados pelo histórico de vendas:\n\n${aiList(s.topClients,x=>`${x.name} — ${money(x.total)} em ${x.count} compra(s), última em ${br(x.last)}`)}\n\nEsses clientes podem receber ofertas, condições especiais ou acompanhamento comercial.`;
  if(q.includes('parado')||q.includes('parados')||q.includes('sem venda'))return s.stoppedCount?`Produtos parados com estoque disponível, consultando vendas e produtos:\n\n${aiList(s.stopped.slice(0,8),p=>`${p.name} — ${p.lastSale?'última venda há '+p.daysWithoutSale+' dias':'sem venda registrada'} • estoque ${Number(p.stock||0)}`)}\n\nSugestão: criar promoção, revisar preço ou reduzir próxima compra desses itens.`:'Não encontrei produtos parados com o critério de 90 dias.';
  if(q.includes('mais vendido')||q.includes('mais vendidos')||q.includes('ranking')||q.includes('produto'))return `Produtos mais vendidos/faturados no período atual:\n\n${aiList(s.topProducts,x=>`${x.name} — ${Number(x.qty||0)} un. • ${money(x.total)}`)}\n\nEsse ranking vem dos itens das vendas registradas no banco.`;
  if(q.includes('mês')||q.includes('mes')||q.includes('lucro')||q.includes('resumo'))return `Resumo do mês consultado no banco:\n\nFaturamento: ${money(s.month)}\nVendas: ${s.monthCount}\nCusto estimado: ${money(s.cost)}\nLucro bruto: ${money(s.gross)}\nDespesas: ${money(s.finance.expensesMonth)}\nLucro líquido estimado: ${money(s.net)}\nEstoque crítico: ${s.lowCount} item(ns)\nContas a receber em aberto: ${money(s.finance.receivableOpen)}`;
  if(q.includes('saúde')||q.includes('saude')){let score=aiHealthScore(s);let label=score>=80?'boa':score>=60?'atenção':'crítica';return `Saúde do negócio: ${score}% (${label}).\n\nBase analisada:\nProdutos cadastrados: ${s.products}\nClientes cadastrados: ${s.clients}\nVendas no histórico: ${s.sales}\nEstoque crítico: ${s.lowCount}\nProdutos parados: ${s.stoppedCount}\nContas vencidas: ${money(s.finance.receivableOverdue)}\nLucro líquido estimado do mês: ${money(s.net)}\n\nQuanto mais vendas registradas, mais precisa fica a análise.`}
  return `Consultei a base local do NexaGest e posso responder sobre vendas, produtos, estoque, clientes e financeiro.\n\nTente perguntar, por exemplo:\n• Quanto vendi hoje?\n• Quais produtos estão com estoque crítico?\n• Quem são meus clientes VIP?\n• Resumo financeiro do mês\n• O que preciso comprar?`;
}
function aiConversationList(){aiEnsureStore();return `<div class="ai-history-list">${db.aiConversations.map(c=>`<button class="${c.id===db.aiActiveConversationId?'active':''}" data-ai-conv="${c.id}"><b>${esc(c.title||'Conversa')}</b><small>${br(c.updatedAt||c.createdAt)}</small></button>`).join('')}</div>`}
function aiMessagesHtml(c){return `<div class="ai-messages" id="aiMessages">${(c.messages||[]).map(m=>`<div class="ai-msg ${m.role==='user'?'user':'assistant'}"><div><b>${m.role==='user'?'Você':'NexaGest AI'}</b><p>${esc(m.text).replace(/\n/g,'<br>')}</p><small>${br(m.date)}</small></div></div>`).join('')}</div>`}
function aiView(){let c=aiActiveConversation();let snap=aiBusinessSnapshot();let score=aiHealthScore(snap);return `<div class="ai-page"><div class="ai-hero panel"><div><span class="pill good">Local</span><h2>🤖 NexaGest AI</h2><p class="muted">Assistente inteligente integrado ao banco local: vendas, produtos, estoque, clientes e financeiro.</p></div><div class="ai-hero-card"><span>Banco conectado</span><b>${snap.products} produtos</b><small>${snap.clients} clientes • ${snap.sales} vendas • saúde ${score}%</small></div></div><div class="grid ai-layout"><aside class="panel ai-sidebar"><div class="between"><h3>Histórico</h3><button class="ghost" id="aiNewConversation">Nova</button></div>${aiConversationList()}<button class="danger full" id="aiClearHistory">Limpar histórico</button></aside><section class="panel ai-chat-panel"><div class="between ai-chat-head"><div><h3>${esc(c.title||'Conversa')}</h3><p class="muted small">Agora o chat consulta os dados reais salvos no NexaGest.</p></div><span class="pill good">DB local</span></div>${aiMessagesHtml(c)}<div class="ai-quick"><b>Perguntas rápidas</b><div>${aiQuickQuestions().map(q=>`<button class="ghost" data-ai-question="${esc(q)}">${esc(q)}</button>`).join('')}</div></div><div class="ai-input-row"><textarea id="aiPrompt" rows="2" placeholder="Digite: resumo do mês, estoque crítico, clientes VIP, vendas de hoje..."></textarea><button class="ok" id="aiSend">Enviar</button></div></section></div><div class="grid cards ai-summary-cards"><div class="card"><span>Vendas hoje</span><b>${money(snap.today)}</b><small>${snap.todayCount} venda(s)</small></div><div class="card"><span>Faturamento mês</span><b>${money(snap.month)}</b><small>${snap.monthCount} venda(s)</small></div><div class="card"><span>Estoque crítico</span><b>${snap.lowCount}</b><small>Produtos para revisar</small></div><div class="card"><span>Financeiro aberto</span><b>${money(snap.finance.receivableOpen)}</b><small>${snap.finance.receivableCount} conta(s)</small></div></div></div>`}
function aiSendMessage(text){let c=aiActiveConversation();let msg=String(text||val('aiPrompt')||'').trim();if(!msg)return;let now=new Date().toISOString();c.messages=c.messages||[];c.messages.push({role:'user',text:msg,date:now});c.messages.push({role:'assistant',text:aiLocalAnswer(msg),date:new Date().toISOString()});c.updatedAt=new Date().toISOString();if(!c.title||c.title==='Nova conversa')c.title=msg.slice(0,38)+(msg.length>38?'...':'');audit('NexaGest AI: consulta ao banco local');save();app();setTimeout(()=>{let box=document.getElementById('aiMessages');if(box)box.scrollTop=box.scrollHeight;document.getElementById('aiPrompt')?.focus()},60)}
function aiNewConversation(){aiEnsureStore();let c={id:uid(),title:'Nova conversa',createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),messages:[{role:'assistant',text:'Nova conversa iniciada. Já posso consultar vendas, produtos, estoque, clientes e financeiro da base local.',date:new Date().toISOString()}]};db.aiConversations.unshift(c);db.aiActiveConversationId=c.id;save();app();setTimeout(()=>document.getElementById('aiPrompt')?.focus(),60)}
function aiClearHistory(){if(!confirm('Limpar todo o histórico do NexaGest AI?'))return;db.aiConversations=[];db.aiActiveConversationId='';aiEnsureStore();audit('Histórico do NexaGest AI limpo');save();app()}
function bindAIEvents(){
  on('aiSend',()=>aiSendMessage());
  on('aiPrompt',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();aiSendMessage()}},'keydown');
  on('aiNewConversation',aiNewConversation);
  on('aiClearHistory',aiClearHistory);
  document.querySelectorAll('[data-ai-question]').forEach(b=>b.onclick=()=>aiSendMessage(b.dataset.aiQuestion));
  document.querySelectorAll('[data-ai-conv]').forEach(b=>b.onclick=()=>{db.aiActiveConversationId=b.dataset.aiConv;save();app()});
}
views.ai=aiView;
const bindBeforeNexaGestAI=bind;
bind=function(){bindBeforeNexaGestAI();bindAIEvents()};


/* IA com inteligência de negócios, alertas e relatórios em linguagem natural */
function aiTrendProducts(days=30,limit=6,mode='up'){
  let sales=aiValidSales(),now=Date.now(),cut=days*86400000,prevCut=days*2*86400000,cur={},prev={};
  aiSaleItems(sales).forEach(i=>{
    let id=i.productId||i.id||i.product_id;if(!id||!i.sale?.date)return;
    let age=now-new Date(i.sale.date).getTime(),qty=Number(i.qty||i.quantity||1),total=Number(i.total||0);
    let bucket=age<=cut?cur:(age<=prevCut?prev:null);if(!bucket)return;
    if(!bucket[id])bucket[id]={id,name:aiProductName(id),qty:0,total:0};bucket[id].qty+=qty;bucket[id].total+=total;
  });
  let ids=new Set([...Object.keys(cur),...Object.keys(prev)]);
  return [...ids].map(id=>{let c=cur[id]||{qty:0,total:0,name:aiProductName(id)},p=prev[id]||{qty:0,total:0};let diff=c.qty-p.qty;let pct=p.qty?Math.round((diff/p.qty)*100):(c.qty>0?100:0);return{id,name:c.name,qty:c.qty,prevQty:p.qty,total:c.total,diff,pct}})
    .filter(x=>mode==='up'?x.diff>0:x.diff<0)
    .sort((a,b)=>mode==='up'?b.diff-a.diff:a.diff-b.diff)
    .slice(0,limit)
}
function aiClientRisk(days=60,limit=8){
  let sales=aiValidSales(),map={};
  sales.forEach(s=>{let id=s.clientId;if(!id||id==='balcao')return;if(!map[id])map[id]={id,name:aiClientName(id),total:0,count:0,last:s.date};map[id].total+=Number(s.total||0);map[id].count++;if(String(s.date||'')>String(map[id].last||''))map[id].last=s.date});
  return Object.values(map).map(x=>({...x,days:aiDaysBetween(x.last)})).filter(x=>x.total>0&&x.days>=days).sort((a,b)=>b.total-a.total).slice(0,limit)
}
function aiBusinessSuggestions(){
  let s=aiBusinessSnapshot(),tips=[],reorder=aiReorderPlan(s),up=aiTrendProducts(30,3,'up'),down=aiTrendProducts(30,3,'down'),risk=aiClientRisk(60,4);
  if(reorder.length)tips.push({level:'Alta',icon:'🛒',title:'Comprar itens críticos',text:`Priorize ${reorder.slice(0,3).map(x=>x.p.name).join(', ')}. O cálculo usa estoque mínimo e vendas recentes.`,action:'Gerar plano de compras'});
  if(s.stoppedCount)tips.push({level:'Média',icon:'🏷️',title:'Criar promoção para produtos parados',text:`Há ${s.stoppedCount} produto(s) com estoque e sem venda recente. Revise preço, exposição ou próxima compra.`,action:'Ver produtos parados'});
  if(risk.length)tips.push({level:'Média',icon:'👥',title:'Reativar clientes importantes',text:`${risk.length} cliente(s) de bom histórico estão sem comprar há mais de 60 dias.`,action:'Montar campanha de reativação'});
  if(s.finance.receivableOverdue>0)tips.push({level:'Alta',icon:'💰',title:'Cobrar contas vencidas',text:`Existem ${money(s.finance.receivableOverdue)} em contas vencidas/atrasadas.`,action:'Abrir financeiro'});
  if(up.length)tips.push({level:'Baixa',icon:'📈',title:'Aproveitar produtos em alta',text:`Itens em alta: ${up.map(x=>x.name).join(', ')}. Considere aumentar estoque e destaque nas vendas.`,action:'Ver tendências'});
  if(down.length)tips.push({level:'Baixa',icon:'📉',title:'Investigar queda de vendas',text:`Itens em queda: ${down.map(x=>x.name).join(', ')}. Compare preço, estoque e demanda.`,action:'Analisar queda'});
  if(!tips.length)tips.push({level:'Boa',icon:'✅',title:'Operação estável',text:'Não encontrei ações urgentes com os dados atuais. Continue registrando vendas para melhorar as análises.',action:'Acompanhar dashboard'});
  return tips;
}
function aiSmartAlerts(){
  let s=aiBusinessSnapshot(),alerts=[];
  if(s.lowCount>0)alerts.push({type:'danger',icon:'🔴',title:'Estoque crítico',text:`${s.lowCount} produto(s) abaixo ou igual ao mínimo.`});
  if(s.finance.receivableOverdue>0)alerts.push({type:'danger',icon:'🔴',title:'Contas vencidas',text:`${money(s.finance.receivableOverdue)} em aberto e vencido.`});
  if(s.net<0)alerts.push({type:'danger',icon:'🔴',title:'Lucro líquido negativo',text:`Resultado estimado do mês: ${money(s.net)}.`});
  if(s.stoppedCount>0)alerts.push({type:'warn',icon:'🟠',title:'Produtos parados',text:`${s.stoppedCount} item(ns) com estoque e pouca movimentação.`});
  if(s.monthCount===0)alerts.push({type:'warn',icon:'🟡',title:'Sem vendas no mês',text:'Ainda não existem vendas registradas no mês atual.'});
  if(alerts.length<4&&s.month>0)alerts.push({type:'good',icon:'🟢',title:'Vendas do mês registradas',text:`Faturamento atual: ${money(s.month)} em ${s.monthCount} venda(s).`});
  return alerts;
}
function aiNaturalBusinessReport(kind='executivo'){
  let s=aiBusinessSnapshot(),score=aiHealthScore(s),tips=aiBusinessSuggestions(),alerts=aiSmartAlerts(),up=aiTrendProducts(30,5,'up'),down=aiTrendProducts(30,5,'down'),risk=aiClientRisk(60,5);
  let lines=[];
  lines.push(`Relatório ${kind} gerado pelo NexaGest AI em ${new Date().toLocaleString('pt-BR')}.`);
  lines.push('');
  lines.push(`Resumo geral: o negócio está com saúde comercial de ${score}%. No mês atual foram registradas ${s.monthCount} venda(s), faturamento de ${money(s.month)}, lucro bruto de ${money(s.gross)} e lucro líquido estimado de ${money(s.net)}.`);
  lines.push(`Hoje: ${s.todayCount} venda(s), totalizando ${money(s.today)}.`);
  lines.push('');
  lines.push('Principais indicadores:');
  lines.push(`• Ticket médio do mês: ${money(s.ticket)}`);
  lines.push(`• Produtos cadastrados: ${s.products}`);
  lines.push(`• Clientes cadastrados: ${s.clients}`);
  lines.push(`• Estoque crítico: ${s.lowCount} item(ns)`);
  lines.push(`• Produtos parados: ${s.stoppedCount} item(ns)`);
  lines.push(`• Contas a receber em aberto: ${money(s.finance.receivableOpen)}`);
  lines.push(`• Contas vencidas: ${money(s.finance.receivableOverdue)}`);
  lines.push('');
  lines.push('Alertas inteligentes:');
  lines.push(alerts.map(a=>`• ${a.icon} ${a.title}: ${a.text}`).join('\n')||'• Nenhum alerta importante encontrado.');
  lines.push('');
  lines.push('Sugestões automáticas:');
  lines.push(tips.slice(0,6).map(t=>`• ${t.icon} ${t.title}: ${t.text}`).join('\n'));
  lines.push('');
  lines.push('Produtos em alta:');
  lines.push(up.length?up.map(x=>`• ${x.name}: +${x.diff} un. em relação ao período anterior`).join('\n'):'• Sem tendência clara de alta com os dados atuais.');
  lines.push('');
  lines.push('Produtos em queda:');
  lines.push(down.length?down.map(x=>`• ${x.name}: ${x.diff} un. em relação ao período anterior`).join('\n'):'• Sem tendência clara de queda com os dados atuais.');
  lines.push('');
  lines.push('Clientes para reativar:');
  lines.push(risk.length?risk.map(x=>`• ${x.name}: ${money(x.total)} no histórico, última compra há ${x.days} dias`).join('\n'):'• Nenhum cliente relevante em risco encontrado.');
  lines.push('');
  lines.push('Conclusão: priorize os alertas vermelhos, reponha itens críticos, cobre valores vencidos e use os produtos em alta para campanhas ou destaque no PDV.');
  return lines.join('\n');
}
function aiExportBusinessReport(){
  let text=aiNaturalBusinessReport('executivo');
  download(text,'nexagest-ai-relatorio-negocios.txt','text/plain;charset=utf-8');
  audit('NexaGest AI: relatório de negócios exportado');
}
function aiSuggestionsHtml(){let tips=aiBusinessSuggestions();return `<div class="panel ai-business-panel"><div class="between"><div><h3>💡 Sugestões automáticas</h3><p class="muted small">Ações recomendadas pela IA com base em vendas, estoque, clientes e financeiro.</p></div><span class="pill good">${tips.length}</span></div><div class="ai-business-list">${tips.map(t=>`<div class="ai-business-item"><b>${t.icon} ${esc(t.title)}</b><span>${esc(t.text)}</span><small>Prioridade: ${esc(t.level)} • ${esc(t.action)}</small></div>`).join('')}</div></div>`}
function aiAlertsHtml(){let alerts=aiSmartAlerts();return `<div class="panel ai-business-panel"><div class="between"><div><h3>🚨 Alertas inteligentes</h3><p class="muted small">Riscos e pontos de atenção detectados automaticamente.</p></div><span class="pill ${alerts.some(a=>a.type==='danger')?'bad':alerts.some(a=>a.type==='warn')?'warn':'good'}">${alerts.length}</span></div><div class="ai-alert-list">${alerts.map(a=>`<div class="ai-alert-item ${esc(a.type)}"><b>${a.icon} ${esc(a.title)}</b><span>${esc(a.text)}</span></div>`).join('')}</div></div>`}
function aiReportsHtml(){return `<div class="panel ai-business-panel"><div class="between"><div><h3>📋 Relatórios em linguagem natural</h3><p class="muted small">Gere um resumo executivo automático e use no atendimento, gestão ou impressão.</p></div><span class="pill">NLP local</span></div><div class="row wrap"><button class="ok" id="aiGenerateBusinessReport">Gerar relatório no chat</button><button class="ghost" id="aiExportBusinessReport">Exportar .txt</button><button class="ghost" data-ai-question="Faça um relatório executivo do meu negócio">Perguntar no chat</button></div></div>`}
const aiQuickQuestionsV1152=aiQuickQuestions;
aiQuickQuestions=function(){return ['Faça um relatório executivo do meu negócio','Quais sugestões você recomenda agora?','Quais alertas inteligentes existem?','Quanto vendi hoje?','Resumo financeiro do mês','Quais produtos estão com estoque crítico?','O que preciso comprar?','Quem são meus clientes VIP?','Quais produtos estão parados?','Produtos em alta e em queda']}
const aiLocalAnswerV1151=aiLocalAnswer;
aiLocalAnswer=function(text){
  let q=String(text||'').toLowerCase();
  if(q.includes('relatório')||q.includes('relatorio')||q.includes('executivo')||q.includes('linguagem natural'))return aiNaturalBusinessReport('executivo');
  if(q.includes('sugest')||q.includes('recomenda')||q.includes('ações')||q.includes('acoes'))return 'Sugestões automáticas encontradas:\n\n'+aiBusinessSuggestions().map((t,i)=>`${i+1}. ${t.icon} ${t.title}\nPrioridade: ${t.level}\n${t.text}\nAção: ${t.action}`).join('\n\n');
  if(q.includes('alerta')||q.includes('risco')||q.includes('problema'))return 'Alertas inteligentes detectados:\n\n'+aiSmartAlerts().map((a,i)=>`${i+1}. ${a.icon} ${a.title}\n${a.text}`).join('\n\n');
  if(q.includes('alta')||q.includes('queda')||q.includes('tendência')||q.includes('tendencia')){let up=aiTrendProducts(30,6,'up'),down=aiTrendProducts(30,6,'down');return `Produtos em alta:\n${aiList(up,x=>`${x.name} — +${x.diff} un. vs. período anterior`)}\n\nProdutos em queda:\n${aiList(down,x=>`${x.name} — ${x.diff} un. vs. período anterior`)}`;}
  if(q.includes('reativar')||q.includes('clientes em risco')){let rows=aiClientRisk(60,8);return `Clientes para reativar:\n\n${aiList(rows,x=>`${x.name} — ${money(x.total)} no histórico, ${x.count} compra(s), última há ${x.days} dias`)}`;}
  return aiLocalAnswerV1151(text);
}
aiView=function(){let c=aiActiveConversation();let snap=aiBusinessSnapshot();let score=aiHealthScore(snap);return `<div class="ai-page"><div class="ai-hero panel"><div><span class="pill good">Local</span><h2>🤖 NexaGest AI</h2><p class="muted">Assistente com inteligência de negócios: sugestões automáticas, alertas inteligentes e relatórios em linguagem natural.</p></div><div class="ai-hero-card"><span>Inteligência ativa</span><b>${score}% saúde</b><small>${snap.products} produtos • ${snap.clients} clientes • ${snap.sales} vendas</small></div></div><div class="grid ai-layout"><aside class="panel ai-sidebar"><div class="between"><h3>Histórico</h3><button class="ghost" id="aiNewConversation">Nova</button></div>${aiConversationList()}<button class="danger full" id="aiClearHistory">Limpar histórico</button></aside><section class="panel ai-chat-panel"><div class="between ai-chat-head"><div><h3>${esc(c.title||'Conversa')}</h3><p class="muted small">A IA consulta o banco local e gera análises comerciais em linguagem natural.</p></div><span class="pill good">BI + IA local</span></div>${aiMessagesHtml(c)}<div class="ai-quick"><b>Perguntas rápidas</b><div>${aiQuickQuestions().map(q=>`<button class="ghost" data-ai-question="${esc(q)}">${esc(q)}</button>`).join('')}</div></div><div class="ai-input-row"><textarea id="aiPrompt" rows="2" placeholder="Digite: relatório executivo, sugestões, alertas, estoque crítico, clientes VIP..."></textarea><button class="ok" id="aiSend">Enviar</button></div></section></div><div class="grid cards ai-summary-cards"><div class="card"><span>Vendas hoje</span><b>${money(snap.today)}</b><small>${snap.todayCount} venda(s)</small></div><div class="card"><span>Faturamento mês</span><b>${money(snap.month)}</b><small>${snap.monthCount} venda(s)</small></div><div class="card"><span>Lucro líquido estimado</span><b>${money(snap.net)}</b><small>Depois de custos e despesas</small></div><div class="card"><span>Alertas</span><b>${aiSmartAlerts().length}</b><small>Riscos e oportunidades</small></div></div><div class="grid two">${aiSuggestionsHtml()}${aiAlertsHtml()}</div>${aiReportsHtml()}</div>`}
const bindAIEventsV1151=bindAIEvents;
bindAIEvents=function(){bindAIEventsV1151();on('aiGenerateBusinessReport',()=>aiSendMessage('Faça um relatório executivo do meu negócio'));on('aiExportBusinessReport',aiExportBusinessReport)};
views.ai=aiView;

/* Configurações internas do Assistente Local */
function aiDefaults(){return {provider:db?.settings?.aiProvider||'local',model:db?.settings?.aiModel||'gpt-4o-mini',apiKey:db?.settings?.aiApiKey||'',endpoint:db?.settings?.aiEndpoint||'',baseUrl:db?.settings?.aiBaseUrl||'http://127.0.0.1:11434',temperature:db?.settings?.aiTemperature??0.3,voiceEnabled:db?.settings?.aiVoiceEnabled!==false,autoMemory:db?.settings?.aiAutoMemory!==false}}
function aiEnsureAdvancedStore(){aiEnsureStore();db.aiMemory=db.aiMemory||[];db.aiProviderLogs=db.aiProviderLogs||[];db.settings.aiProvider=db.settings.aiProvider||'local';db.settings.aiModel=db.settings.aiModel||'gpt-4o-mini';db.settings.aiBaseUrl=db.settings.aiBaseUrl||'http://127.0.0.1:11434';db.settings.aiEndpoint=db.settings.aiEndpoint||'';db.settings.aiTemperature=db.settings.aiTemperature??0.3;db.settings.aiVoiceEnabled=db.settings.aiVoiceEnabled!==false;db.settings.aiAutoMemory=db.settings.aiAutoMemory!==false;return db.aiMemory}
function aiProviderName(){return 'Assistente Local'}
function aiProviderReady(){return true}
function aiProviderStatusPill(){return '<span class="pill good">100% local</span>'}
function aiBusinessContextText(){let snap=aiBusinessSnapshot(),score=aiHealthScore(snap),alerts=aiSmartAlerts().slice(0,5),tips=aiBusinessSuggestions().slice(0,5),low=aiLowStock().slice(0,8),top=aiTopProducts(aiValidSales().filter(s=>aiDaysBetween(s.date)<=30),5),vip=aiTopClients(aiValidSales(),5);return [
`Empresa: ${db.settings.company||'NexaGest'}. Data atual: ${new Date().toLocaleString('pt-BR')}.`,
`Resumo: ${snap.products} produtos, ${snap.clients} clientes, ${snap.sales} vendas, saúde comercial ${score}%.`,
`Hoje: ${money(snap.today)} em ${snap.todayCount} venda(s). Mês: ${money(snap.month)} em ${snap.monthCount} venda(s). Lucro líquido estimado: ${money(snap.net)}.`,
`Financeiro aberto: ${money(snap.finance.receivableOpen)}; vencido: ${money(snap.finance.receivableOverdue)}.`,
`Estoque crítico: ${low.map(p=>`${p.name} (${p.stock}/${p.min})`).join(', ')||'nenhum'}.`,
`Top produtos 30 dias: ${top.map(x=>`${x.name} ${x.qty} un. ${money(x.total)}`).join('; ')||'sem vendas'}.`,
`Clientes VIP: ${vip.map(x=>`${x.name} ${money(x.total)}`).join('; ')||'sem clientes'}.`,
`Alertas: ${alerts.map(a=>`${a.title}: ${a.text}`).join(' | ')||'sem alertas'}.`,
`Sugestões: ${tips.map(t=>`${t.title}: ${t.action}`).join(' | ')||'sem sugestões'}.`
].join('\n')}
function aiMemoryText(){aiEnsureAdvancedStore();let rows=(db.aiMemory||[]).slice(0,12);return rows.length?rows.map((m,i)=>`${i+1}. ${m.text}`).join('\n'):'Sem memória salva ainda.'}
function aiMaybeCaptureMemory(text){aiEnsureAdvancedStore();if(!db.settings.aiAutoMemory)return;let raw=String(text||'').trim();let lower=raw.toLowerCase();let wants=/(lembre|memorize|guarde|salve na memória|salve na memoria|não esqueça|nao esqueca)/i.test(raw);if(!wants)return;let cleaned=raw.replace(/^(por favor,?\s*)?(lembre|memorize|guarde|salve na memória|salve na memoria|não esqueça|nao esqueca)( que|:)?/i,'').trim();if(cleaned.length<4)cleaned=raw;db.aiMemory.unshift({id:uid(),text:cleaned,date:new Date().toISOString(),source:'chat'});db.aiMemory=db.aiMemory.slice(0,40)}
function aiAddAutoMemoryFromAnswer(question,answer){aiEnsureAdvancedStore();if(!db.settings.aiAutoMemory)return;let q=String(question||'').toLowerCase();if(q.includes('relatório')||q.includes('relatorio')||q.includes('sugest')||q.includes('alerta')){db.aiMemory.unshift({id:uid(),text:`Última análise importante: ${String(answer||'').split('\n').slice(0,2).join(' ').slice(0,220)}`,date:new Date().toISOString(),source:'auto'});db.aiMemory=db.aiMemory.slice(0,40)}}
function aiBuildMessages(prompt){return [{role:'system',content:'Você é o NexaGest AI, assistente interno de um sistema desktop de gestão comercial no Brasil. Responda em português do Brasil, de forma objetiva, prática e segura. Use os dados do contexto local como fonte principal. Quando faltarem dados, diga isso claramente e recomende próximos passos. Não invente números.'},{role:'system',content:'CONTEXTO DO NEGÓCIO:\n'+aiBusinessContextText()},{role:'system',content:'MEMÓRIA DE CONTEXTO:\n'+aiMemoryText()},...((aiActiveConversation().messages||[]).slice(-8).map(m=>({role:m.role==='user'?'user':'assistant',content:m.text||''}))),{role:'user',content:String(prompt||'')}];}
async function aiAdvancedAnswer(prompt){aiEnsureAdvancedStore();aiMaybeCaptureMemory(prompt);let cfg=aiDefaults();if(cfg.provider==='local'||!window.nexagest?.aiProviderRequest){let answer=aiLocalAnswer(prompt);aiAddAutoMemoryFromAnswer(prompt,answer);return answer+'\n\n🧠 Modo usado: IA local do NexaGest.'}
try{
  let payload={provider:cfg.provider,model:cfg.model,apiKey:cfg.apiKey,endpoint:cfg.endpoint,baseUrl:cfg.baseUrl,temperature:Number(cfg.temperature||0.3),messages:aiBuildMessages(prompt)};
  let res=await window.nexagest.aiProviderRequest(payload);
  db.aiProviderLogs=db.aiProviderLogs||[];db.aiProviderLogs.unshift({id:uid(),date:new Date().toISOString(),provider:cfg.provider,model:cfg.model,ok:!!res?.ok,message:res?.message||''});db.aiProviderLogs=db.aiProviderLogs.slice(0,30);
  if(res?.ok&&String(res.text||'').trim()){let answer=String(res.text||'').trim()+`\n\n🤖 Modo usado: ${aiProviderName()} (${cfg.model||'modelo padrão'}).`;aiAddAutoMemoryFromAnswer(prompt,answer);return answer;}
  let fallback=aiLocalAnswer(prompt);return `${fallback}\n\n⚠️ Não consegui usar ${aiProviderName()} agora (${res?.message||'sem resposta'}). Usei a IA local do NexaGest como fallback.`;
}catch(e){let fallback=aiLocalAnswer(prompt);return `${fallback}\n\n⚠️ Falha ao consultar ${aiProviderName()}: ${e.message||e}. Usei a IA local do NexaGest como fallback.`}
}
function aiProviderSettingsHtml(){return `<div class="panel ai-business-panel ai-provider-panel"><div class="between"><div><h3>⚙️ Assistente Local</h3><p class="muted small">A IA funciona apenas dentro do NexaGest, usando dados locais e conhecimento do próprio app.</p></div><span class="pill good">Offline</span></div></div>`}
function aiMemoryHtml(){aiEnsureAdvancedStore();let rows=db.aiMemory||[];return `<div class="panel ai-business-panel"><div class="between"><div><h3>🧠 Memória de contexto</h3><p class="muted small">Informações úteis salvas para personalizar respostas futuras.</p></div><span class="pill">${rows.length}</span></div><div class="ai-memory-list">${rows.length?rows.slice(0,8).map(m=>`<div><b>${esc(m.source==='auto'?'Automática':'Manual')}</b><span>${esc(m.text)}</span><small>${br(m.date)}</small></div>`).join(''):'<p class="muted">Diga no chat: “lembre que...” para salvar uma memória.</p>'}</div><div class="row wrap"><input id="aiMemoryManual" placeholder="Adicionar memória manual"><button class="ghost" id="aiAddMemory">Adicionar</button><button class="danger" id="aiClearMemory">Limpar memória</button></div></div>`}
function aiVoiceHtml(){return ''}
function aiAssistantCompleteHtml(){return `<div class="panel ai-business-panel"><div class="between"><div><h3>✅ Assistente Local completo</h3><p class="muted small">Foco em suporte interno do NexaGest, status do sistema e consultas locais.</p></div></div></div>`}
const aiQuickQuestionsV1153=aiQuickQuestions;
aiQuickQuestions=function(){return ['Faça um relatório executivo usando IA avançada','Quais ações eu devo tomar hoje?','Analise meu estoque e sugira compras','Quais clientes devo reativar?','Crie uma mensagem para clientes VIP','Quais alertas inteligentes existem?','Quanto vendi hoje?','Resumo financeiro do mês','Produtos em alta e em queda','Lembre que meu foco é vender mais produtos lucrativos']}
const aiSendMessageBeforeAdvanced=aiSendMessage;
aiSendMessage=async function(text){let c=aiActiveConversation();let msg=String(text||val('aiPrompt')||'').trim();if(!msg)return;let now=new Date().toISOString();c.messages=c.messages||[];c.messages.push({role:'user',text:msg,date:now});c.messages.push({role:'assistant',text:'Pensando com o NexaGest AI...',date:new Date().toISOString(),pending:true});c.updatedAt=new Date().toISOString();if(!c.title||c.title==='Nova conversa')c.title=msg.slice(0,38)+(msg.length>38?'...':'');save();app();setTimeout(()=>{let box=document.getElementById('aiMessages');if(box)box.scrollTop=box.scrollHeight},30);let answer=await aiAdvancedAnswer(msg);let pending=[...(c.messages||[])].reverse().find(m=>m.pending);if(pending){pending.text=answer;delete pending.pending;pending.date=new Date().toISOString()}else c.messages.push({role:'assistant',text:answer,date:new Date().toISOString()});c.updatedAt=new Date().toISOString();audit('NexaGest AI: consulta avançada');save();app();setTimeout(()=>{let box=document.getElementById('aiMessages');if(box)box.scrollTop=box.scrollHeight;document.getElementById('aiPrompt')?.focus()},60)}
aiMessagesHtml=function(c){return `<div class="ai-messages" id="aiMessages">${(c.messages||[]).map(m=>`<div class="ai-msg ${m.role==='user'?'user':'assistant'} ${m.pending?'pending':''}"><div><b>${m.role==='user'?'Você':'NexaGest AI'}</b><p>${esc(m.text).replace(/\n/g,'<br>')}</p><small>${br(m.date)}</small></div></div>`).join('')}</div>`}
aiView=function(){aiEnsureAdvancedStore();let c=aiActiveConversation();let snap=aiBusinessSnapshot();let score=aiHealthScore(snap);return `<div class="ai-page"><div class="ai-hero panel"><div><span class="pill good">Local</span><h2>🤖 NexaGest AI Avançada</h2><p class="muted">Assistente Local com memória de contexto, banco local e funcionamento offline.</p></div><div class="ai-hero-card"><span>${esc(aiProviderName())}</span><b>${score}% saúde</b><small>${snap.products} produtos • ${snap.clients} clientes • ${snap.sales} vendas</small></div></div><div class="grid ai-layout"><aside class="panel ai-sidebar"><div class="between"><h3>Histórico</h3><button class="ghost" id="aiNewConversation">Nova</button></div>${aiConversationList()}<button class="danger full" id="aiClearHistory">Limpar histórico</button></aside><section class="panel ai-chat-panel"><div class="between ai-chat-head"><div><h3>${esc(c.title||'Conversa')}</h3><p class="muted small">A IA usa dados locais, memória e ${esc(aiProviderName())} quando configurado.</p></div>${aiProviderStatusPill()}</div>${aiMessagesHtml(c)}<div class="ai-quick"><b>Perguntas rápidas</b><div>${aiQuickQuestions().map(q=>`<button class="ghost" data-ai-question="${esc(q)}">${esc(q)}</button>`).join('')}</div></div><div class="ai-input-row"><textarea id="aiPrompt" rows="2" placeholder="Digite: analise vendas, estoque, clientes, financeiro..."></textarea><button class="ok" id="aiSend">Enviar</button></div></section></div><div class="grid cards ai-summary-cards"><div class="card"><span>Vendas hoje</span><b>${money(snap.today)}</b><small>${snap.todayCount} venda(s)</small></div><div class="card"><span>Faturamento mês</span><b>${money(snap.month)}</b><small>${snap.monthCount} venda(s)</small></div><div class="card"><span>Provedor</span><b>${esc(aiProviderName())}</b><small>${aiProviderReady()?'Pronto':'Fallback local ativo'}</small></div><div class="card"><span>Memórias</span><b>${(db.aiMemory||[]).length}</b><small>Contexto salvo</small></div></div><div class="grid two">${aiSuggestionsHtml()}${aiAlertsHtml()}</div><div class="grid two">${aiProviderSettingsHtml()}${aiMemoryHtml()}</div>${aiAssistantCompleteHtml()}${aiReportsHtml()}</div>`}
function aiSaveSettings(){aiEnsureAdvancedStore();db.settings.aiProvider=val('aiProvider')||'local';db.settings.aiModel=val('aiModel')||'';db.settings.aiApiKey=val('aiApiKey')||'';db.settings.aiEndpoint=val('aiEndpoint')||'';db.settings.aiBaseUrl=val('aiBaseUrl')||'http://127.0.0.1:11434';db.settings.aiTemperature=Number(val('aiTemperature')||0.3);db.settings.aiAutoMemory=!!document.getElementById('aiAutoMemory')?.checked;db.settings.aiVoiceEnabled=!!document.getElementById('aiVoiceEnabled')?.checked;audit('Configurações da NexaGest AI salvas');save();app();alert('Configurações da IA salvas.')}
function aiAddMemory(){aiEnsureAdvancedStore();let text=val('aiMemoryManual').trim();if(!text)return;db.aiMemory.unshift({id:uid(),text,date:new Date().toISOString(),source:'manual'});db.aiMemory=db.aiMemory.slice(0,40);save();app()}
function aiClearMemory(){if(!confirm('Limpar a memória de contexto da IA?'))return;db.aiMemory=[];save();app()}

const bindAIEventsBeforeAdvanced=bindAIEvents;
bindAIEvents=function(){bindAIEventsBeforeAdvanced();on('aiSaveSettings',aiSaveSettings);on('aiAddMemory',aiAddMemory);on('aiClearMemory',aiClearMemory);on('aiGenerateBusinessReport',()=>aiSendMessage('Faça um relatório executivo do meu negócio usando todos os dados disponíveis'));on('aiExportBusinessReport',aiExportBusinessReport)};
views.ai=aiView;


/* Assistente Local do NexaGest */
function aiLicenseStatusText(){
  try{
    const st=effectiveLicenseState();
    const tipo=st.type==='permanent'?'Permanente':(st.type==='trial'?'Teste automático':'Por período');
    const status=st.ok===false?'Expirada':(st.status==='pending'?'Pendente de ativação':'Ativa');
    const owner=db.settings?.licenseOwner||st.owner||'—';
    const email=db.settings?.licenseEmail||st.email||'—';
    const validade=st.type==='permanent'?'Permanente':formatLicenseDate(st.expiresAt||db.settings?.licenseExpiresAt);
    const ativada=formatLicenseDate(st.activatedAt||db.settings?.licenseActivatedAt);
    const restante=st.type==='permanent'?'Sem vencimento':formatLicenseRemaining(st);
    return [`Status: ${status}`,`Tipo: ${tipo}`,`Cliente: ${owner}`,`E-mail: ${email}`,`Ativada em: ${ativada}`,`Validade: ${validade}`,`Tempo restante: ${restante}`].join('\n');
  }catch(e){return 'Não consegui ler o status da licença agora.'}
}
function aiLicenseHelpText(){
  return `Licenciamento do NexaGest:\n\n• Ao instalar pela primeira vez, o sistema cria um teste automático de 8 dias.\n• Quando faltar 3 dias ou menos, aparece um aviso global ao lado do relógio.\n• Se a licença vencer, o sistema bloqueia o uso, mas preserva todos os dados.\n• Existem licenças por período e permanentes.\n• O cliente cola apenas a chave de licença; nome e e-mail vêm dentro da licença gerada no License Manager DEV.\n• O License Manager DEV permite gerar, copiar, renovar, adicionar dias, transformar em permanente, editar base, excluir e exportar histórico.\n• O instalador e o publicador fazem parte do fluxo comercial do NexaGest 1.0.0.`;
}
function aiCommercialSystemOverview(){
  return `NexaGest 1.0.0 — Primeira versão comercial:\n\n• PDV/Caixa com finalização de venda, comprovante, impressão, HTML/PDF e WhatsApp.\n• Produtos, estoque, clientes, fornecedores, compras, financeiro e relatórios.\n• Dashboard, BI, Curva ABC, alertas e inteligência comercial.\n• Cloud, backup, rede local e Multi-PC já estruturados.\n• NexaGest AI local para suporte interno e consultas ao banco.\n• Licenciamento offline com trial automático, licenças por período/permanente, bloqueio e License Manager DEV.\n• Comprovantes trazem o aviso: Este documento é um comprovante de venda. Não possui valor fiscal.`;
}
function aiLocalCapabilities(){
  return [
    ['dashboard','📊 Dashboard','Indicadores, vendas do dia, faturamento, lucro, produtos em destaque e visão gerencial.'],
    ['pdv','💵 Caixa / PDV','Vendas, abertura e fechamento de caixa, formas de pagamento e modo operador.'],
    ['products','📦 Produtos','Cadastro, preço, custo, estoque, código de barras, categorias e situação ativa/inativa.'],
    ['stock','🏷️ Estoque','Movimentações, entradas, saídas, estoque crítico e produtos parados.'],
    ['clients','👥 Clientes','Cadastro, histórico, clientes VIP e clientes para reativar.'],
    ['finance','💰 Financeiro','Despesas, contas a receber, fiado, vencimentos e resumo financeiro.'],
    ['purchases','🧾 Compras','Compras, fornecedores, reposição e plano de compras.'],
    ['reports','📈 Relatórios','Relatórios gerenciais, exportações e análises.'],
    ['premium','⭐ BI','Curva ABC, indicadores, comparativos, painel executivo e análises avançadas.'],
    ['cloud','☁️ Cloud','Backup, sincronização, multi-PC e status da nuvem/local.'],
    ['license','🔐 Licenciamento','Trial automático, licença por período, licença permanente, avisos globais e bloqueio ao vencer.'],
    ['dev','🛠️ License Manager DEV','Geração e gerenciamento offline de licenças, renovação, adicionar dias, permanente e histórico.'],
    ['receipts','🧾 Comprovantes','Comprovante de venda por impressão, HTML/PDF e WhatsApp, com aviso de que não possui valor fiscal.'],
    ['settings','⚙️ Configurações','Empresa, sistema, usuários, banco, backup local e preferências.']
  ];
}
function aiModuleStatusHtml(){let caps=aiLocalCapabilities();return `<div class="panel ai-business-panel"><div class="between"><div><h3>🧭 Módulos do NexaGest</h3><p class="muted small">Mapa interno do sistema para o assistente explicar onde fica cada função.</p></div><span class="pill good">Local</span></div><div class="ai-capability-grid">${caps.map(c=>`<div><b>${c[1]}</b><span>${esc(c[2])}</span></div>`).join('')}</div></div>`}
function aiLocalSystemStatus(){
  let snap=aiBusinessSnapshot(), cash=currentCashRegister?currentCashRegister():null;
  let cloud=db.settings||{};
  ...
  return [
    ...
    `Licença: ${formatLicenseRemaining(effectiveLicenseState())}`,
    ...
    `Clientes cadastrados: ${snap.clients}`,
    `Vendas registradas: ${snap.sales}`,
    `Estoque crítico: ${snap.lowCount} produto(s)`,
    `Faturamento do mês: ${money(snap.month)}`,
    `Contas a receber em aberto: ${money(snap.finance.receivableOpen)}`,
    `Cloud: ${cloud.cloudEnabled?'ativado':'não ativado / local'}${cloud.lastSyncAt?' • última sincronização '+br(cloud.lastSyncAt):''}`,
    `Backup: ${cloud.lastBackupAt?'último backup '+br(cloud.lastBackupAt):'sem backup recente registrado'}`
  ].join('\n');
}
function aiExplainFeature(q){
  let txt=String(q||'').toLowerCase();
  if(txt.includes('licença')||txt.includes('licenca')||txt.includes('trial')||txt.includes('bloqueio')||txt.includes('license manager')||txt.includes('ativar')){
    return aiLicenseHelpText()+'\n\nStatus atual da licença:\n'+aiLicenseStatusText();
  }
  if(txt.includes('comprovante')||txt.includes('cupom')||txt.includes('fiscal')||txt.includes('whatsapp')){
    return `Comprovantes no NexaGest:\n\n• Após finalizar uma venda, o sistema abre a tela de comprovante.\n• O comprovante pode ser impresso, salvo em HTML/PDF ou enviado pelo WhatsApp.\n• A tela do comprovante fecha pelo botão Esc ou pela tecla ESC do teclado.\n• O comprovante inclui o aviso: Este documento é um comprovante de venda. Não possui valor fiscal.\n• Para NFC-e/cupom fiscal real seria necessário módulo fiscal, certificado e SEFAZ, mas o NexaGest 1.0.0 trabalha com comprovante de venda não fiscal.`;
  }
  if(txt.includes('pronto')||txt.includes('venda comercial')||txt.includes('versão 1')||txt.includes('versao 1')||txt.includes('primeira versão')||txt.includes('primeira versao')){
    return aiCommercialSystemOverview();
  }
  let rows=aiLocalCapabilities();
  let found=rows.find(r=>txt.includes(r[0])||txt.includes(r[1].toLowerCase().replace(/[^a-zà-ú0-9 ]/gi,'').split(' ')[0])||txt.includes(r[1].toLowerCase().replace(/[^a-zà-ú0-9 ]/gi,'')));
  if(txt.includes('backup'))return 'Backup no NexaGest:\n\n• Backup local fica em Configurações/Backup.\n• Backup Cloud fica na tela Cloud.\n• Use backup antes de atualizar ou trocar de computador.\n• Na versão atual, o assistente apenas orienta e mostra status; ele não envia seus dados para internet.';
  if(txt.includes('sincron')||txt.includes('multi-pc')||txt.includes('multi pc')||txt.includes('cloud'))return 'Cloud / Sincronização / Multi-PC:\n\nA tela Cloud centraliza sincronização, backup cloud e uso em mais de um computador. O recomendado é deixar Configurações para recursos locais e Cloud para tudo que envolve conexão, servidor ou vários PCs.';
  if(txt.includes('caixa')||txt.includes('pdv')||txt.includes('venda'))return 'Caixa / PDV:\n\n1. Abra o caixa informando o valor inicial.\n2. Pesquise ou escaneie o produto.\n3. Informe quantidade e forma de pagamento.\n4. Finalize a venda.\n5. No fim do período, use Fechar Caixa para conferir dinheiro, Pix, débito e crédito.';
  if(txt.includes('produto')||txt.includes('estoque'))return 'Produtos e estoque:\n\n• Cadastre nome, código de barras, preço de venda, custo e estoque mínimo.\n• O sistema usa o estoque mínimo para gerar alertas de estoque crítico.\n• Produtos sem venda por muito tempo aparecem como produtos parados.\n• O plano de compras usa estoque atual, mínimo e vendas recentes.';
  if(txt.includes('financeiro')||txt.includes('fiado')||txt.includes('receber'))return 'Financeiro:\n\nO módulo financeiro acompanha despesas, contas a receber, fiado, vencimentos e lucro estimado. O assistente consulta esses dados localmente para mostrar resumo do mês e valores em aberto.';
  if(txt.includes('bi')||txt.includes('pro')||txt.includes('curva abc'))return 'BI:\n\nA versão BI mostra análises mais gerenciais, como painel executivo, Curva ABC, comparativos, indicadores, heatmap e rankings. Ele serve para entender quais produtos, clientes e períodos mais impactam o negócio.';
  if(found)return `${found[1]}\n\n${found[2]}\n\nVocê encontra esse módulo no menu lateral do NexaGest. Posso também consultar os dados atuais desse módulo se você perguntar algo específico.`;
  return '';
}
const aiLocalAnswerBefore1154 = aiLocalAnswer;
aiLocalAnswer=function(text){
  let q=String(text||'').toLowerCase();
  if(q.includes('versão')||q.includes('versao')||q.includes('status')||q.includes('como está')||q.includes('como esta')||q.includes('configurado')||q.includes('situação')||q.includes('situacao'))return 'Status local do NexaGest:\n\n'+aiLocalSystemStatus();
  if(q.includes('o que tem')||q.includes('funções')||q.includes('funcoes')||q.includes('módulos')||q.includes('modulos')||q.includes('recursos')||q.includes('telas'))return 'Principais módulos do NexaGest:\n\n'+aiLocalCapabilities().map((x,i)=>`${i+1}. ${x[1]} — ${x[2]}`).join('\n');
  if(q.includes('onde')||q.includes('como faço')||q.includes('como faco')||q.includes('me ajuda')||q.includes('explique')||q.includes('explica')){let ans=aiExplainFeature(q);if(ans)return ans;}
  if(q.includes('internet')||q.includes('openai')||q.includes('api')||q.includes('microfone'))return 'Esta versão foi ajustada para ser um Assistente Local do NexaGest.\n\n• Não pesquisa na internet.\n• Não exige chave ou provedor externo.\n• Responde usando conhecimento interno do app e dados locais do banco.';
  return aiLocalAnswerBefore1154(text);
}
function aiDefaults(){return {provider:'local',model:'nexagest-local-engine',apiKey:'',endpoint:'',baseUrl:'',temperature:0,voiceEnabled:false,autoMemory:db?.settings?.aiAutoMemory!==false}}
function aiEnsureAdvancedStore(){aiEnsureStore();db.aiMemory=db.aiMemory||[];db.aiProviderLogs=db.aiProviderLogs||[];db.settings.aiProvider='local';db.settings.aiVoiceEnabled=false;db.settings.aiAutoMemory=db.settings.aiAutoMemory!==false;return db.aiMemory}
function aiProviderName(){return 'Assistente Local'}
function aiProviderReady(){return true}
function aiProviderStatusPill(){return '<span class="pill good">100% local</span>'}
async function aiAdvancedAnswer(prompt){aiEnsureAdvancedStore();aiMaybeCaptureMemory(prompt);let answer=aiLocalAnswer(prompt);aiAddAutoMemoryFromAnswer(prompt,answer);return answer+'\n\n🧠 Modo usado: Assistente Local do NexaGest (modo local).'}
function aiProviderSettingsHtml(){return `<div class="panel ai-business-panel ai-provider-panel"><div class="between"><div><h3>⚙️ Assistente Local</h3><p class="muted small">A IA foi ajustada para ajudar dentro do NexaGest, usando apenas recursos locais.</p></div><span class="pill good">Offline</span></div><div class="ai-capability-grid"><div><b>Sem internet</b><span>Não pesquisa nada fora do sistema.</span></div><div><b>Sem provedor externo</b><span>Não exige chave nem configuração online.</span></div><div><b>Dados locais</b><span>Consulta vendas, produtos, clientes, estoque e financeiro.</span></div><div><b>Ajuda do app</b><span>Explica módulos, funções, status e configurações.</span></div></div><label class="check-row"><input type="checkbox" id="aiAutoMemory" ${db.settings.aiAutoMemory!==false?'checked':''}> Memória de contexto automática</label><div class="row wrap"><button id="aiSaveSettings">Salvar preferências</button></div></div>`}
function aiVoiceHtml(){return ''}
function aiAssistantCompleteHtml(){return `<div class="panel ai-business-panel"><div class="between"><div><h3>✅ Assistente Local completo</h3><p class="muted small">Foco em suporte interno do NexaGest, status do sistema e consultas locais.</p></div></div><div class="ai-capability-grid"><div><b>Ajuda sobre o app</b><span>Explica telas, módulos e como usar cada recurso.</span></div><div><b>Status do sistema</b><span>Mostra versão, caixa, cloud, backup, produtos e vendas.</span></div><div><b>Consulta dados</b><span>Produtos, clientes, vendas, estoque e financeiro.</span></div><div><b>Modo local</b><span>Atendimento por texto dentro do sistema.</span></div></div></div>`}
aiQuickQuestions=function(){return ['Qual é o status do NexaGest agora?','Como está minha licença?','Como funciona o License Manager DEV?','Quais módulos o sistema tem?','Como faço uma venda no caixa?','Como funciona o comprovante de venda?','Explique o Cloud e Multi-PC','Resumo financeiro do mês','Quais produtos estão com estoque crítico?','Faça um relatório executivo do meu negócio']}
aiView=function(){aiEnsureAdvancedStore();let c=aiActiveConversation();let snap=aiBusinessSnapshot();let score=aiHealthScore(snap);return `<div class="ai-page"><div class="ai-hero panel"><div><span class="pill good">Local</span><h2>🤖 Assistente Local do NexaGest</h2><p class="muted">Ajuda interna do sistema, status dos módulos e consultas ao banco local. Modo local.</p></div><div class="ai-hero-card"><span>Assistente Local</span><b>${score}% saúde</b><small>${snap.products} produtos • ${snap.clients} clientes • ${snap.sales} vendas</small></div></div><div class="grid ai-layout"><aside class="panel ai-sidebar"><div class="between"><h3>Histórico</h3><button class="ghost" id="aiNewConversation">Nova</button></div>${aiConversationList()}<button class="danger full" id="aiClearHistory">Limpar histórico</button></aside><section class="panel ai-chat-panel"><div class="between ai-chat-head"><div><h3>${esc(c.title||'Conversa')}</h3><p class="muted small">Pergunte sobre o app, módulos, status, vendas, estoque, clientes e financeiro.</p></div>${aiProviderStatusPill()}</div>${aiMessagesHtml(c)}<div class="ai-quick"><b>Perguntas rápidas</b><div>${aiQuickQuestions().map(q=>`<button class="ghost" data-ai-question="${esc(q)}">${esc(q)}</button>`).join('')}</div></div><div class="ai-input-row"><textarea id="aiPrompt" rows="2" placeholder="Digite: licença, comprovante, caixa, backup, financeiro, estoque crítico..."></textarea><button class="ok" id="aiSend">Enviar</button></div></section></div><div class="grid cards ai-summary-cards"><div class="card"><span>Vendas hoje</span><b>${money(snap.today)}</b><small>${snap.todayCount} venda(s)</small></div><div class="card"><span>Faturamento mês</span><b>${money(snap.month)}</b><small>${snap.monthCount} venda(s)</small></div><div class="card"><span>Modo IA</span><b>Local</b><small>Modo local</small></div><div class="card"><span>Alertas</span><b>${aiSmartAlerts().length}</b><small>Riscos e oportunidades</small></div></div><div class="grid two">${aiSuggestionsHtml()}${aiAlertsHtml()}</div><div class="grid two">${aiProviderSettingsHtml()}${aiMemoryHtml()}</div>${aiModuleStatusHtml()}${aiReportsHtml()}</div>`}
function aiSaveSettings(){aiEnsureAdvancedStore();db.settings.aiProvider='local';db.settings.aiVoiceEnabled=false;db.settings.aiAutoMemory=!!document.getElementById('aiAutoMemory')?.checked;audit('Preferências do Assistente Local salvas');save();app();alert('Preferências do Assistente Local salvas. A IA está em modo local.')}

const bindAIEventsLocal1154=bindAIEvents;
bindAIEvents=function(){bindAIEventsLocal1154();on('aiSaveSettings',aiSaveSettings);on('aiGenerateBusinessReport',()=>aiSendMessage('Faça um relatório executivo do meu negócio'));on('aiExportBusinessReport',aiExportBusinessReport)};
views.ai=aiView;
