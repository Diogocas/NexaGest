// NexaGest 9.2.1 - Diagnóstico de Atualização
...
/* NexaGest 9.3.0 - Publicação Automática integrado */
...
function updateNotesHtml(notes){
  ...
  if(!text)return '<li>Melhorias e correções da nova versão.</li><li>Atualização segura via GitHub Releases.</li>';
  return text.split(/\r?\n/).map(x=>x.replace(/^[-*•]\s*/, '').trim()).filter(Boolean).slice(0,6).map(x=>`<li>${esc(x)}</li>`).join('') || `<li>${esc(text.slice(0,180))}</li>`;
}
