...
function dashboardActionCenter(d){
  ...
  return `<div class="panel action-center-panel"><div class="between"><div><h3>Central de ações recomendadas</h3><p class="muted small">Lista objetiva do que o gestor deve olhar primeiro.</p></div><span class="pill good">Etapa 2</span></div><div class="action-center-list">${actions.map(a=>`<button data-dash-go="${a[3]}"><b>${esc(a[0])}</b><span>${esc(a[1])}</span><em class="${a[2]==='Alta'?'bad':a[2]==='Média'?'warn':'good'}">${esc(a[2])}</em></button>`).join('')}</div></div>`;
}
...
/* NexaGest 10.0 - Inteligência Comercial */
...
function updateNotesHtml(notes){
  ...
  if(!text){
    return '<li>Atualizador Premium integrado ao NexaGest.</li><li>Correções e melhorias de estabilidade.</li><li>Instalação segura via GitHub Releases.</li>';
  }
  return text.split(/\n+/)
    .map(x=>x.trim())
    .filter(Boolean)
